...
### §5.2 任务完成复盘（每个任务结束时，由执行角色输出）

每个编码任务完成后，执行角色必须输出以下复盘（Stop hook自动提醒）：
1. **设计对照**：实现是否符合DEV文档规格？列出偏差项及原因
2. **质量总结**：测试通过率/ruff状态/已知问题/技术债
3. **改善建议**：发现的流程/工具/规范可改进项（至少1条）
4. **经验教训**：值得记入LESSONS_LEARNED的发现
5. **下一步**：后续任务的依赖/阻塞/风险提示

审查角色（qa-tester/quant-reviewer/risk-guardian）在交叉审查后也必须输出：
- 审查发现（PASS/FAIL + 具体问题）
- 对原实现的改善建议
- 风险评估（该变更对整体系统的影响）

### §5.3 Sprint结束（必做，顺序不可换）
1. **更新PROGRESS.md**（铁律6，验证当前状态——铁律5）
2. spawn复盘agent，参与角色发言（复盘5问+投资人3问）
3. 汇总本Sprint所有任务复盘的改善建议 → 筛选可落地项
4. 经验教训 → LESSONS_LEARNED.md
5. 技术决策 → docs/TECH_DECISIONS.md
6. 规则执行记分卡（违规次数+用户提醒次数）
7. **审计日志审查**：review `.claude/hooks/audit.log`，统计本Sprint操作模式（写入次数/agent spawn次数/被拦截次数），异常模式记入LL
8. **DEVELOPMENT_BLUEPRINT更新**：对照DEV文档刷新功能完成度百分比
9. **输出Sprint完成报告**（§5.6模板，直接输出给用户）
10. Git commit + tag
...
### §5.6 Sprint完成报告模板（必须输出给用户）

Sprint结束时，Team Lead必须按以下模板向用户输出结构化报告：
...
计划任务数: X | 完成: X | 部分完成: X | 未完成: X

### 二、交付物清单
| 交付物 | 类型 | 文件路径 |
|--------|------|---------|
| XxxService | 新增代码 | backend/app/services/xxx.py |
| test_xxx | 新增测试 | backend/tests/test_xxx.py |

### 三、关键指标
- 代码变更: +X行 / -X行 / X个文件
- 测试: 新增X个, 总计X个, 通过率X%
- ruff: PASS/FAIL (X issues)
- 设计完成度: DEVELOPMENT_BLUEPRINT X% → X%

### 四、质量与风险
- 已知问题/技术债: (列表)
- 风险项: (对系统稳定性/性能/正确性的影响)
- 交叉审查结果: qa/quant/risk的关键发现
...
**Phase结束时**（如Phase 0→Phase 1），在Sprint报告基础上额外输出：
- Phase级别目标达成度回顾
- 累计指标汇总（总测试数/总代码行/设计完成度趋势）
- Phase间技术债务清单
- 下一Phase的关键风险和准备状态评估

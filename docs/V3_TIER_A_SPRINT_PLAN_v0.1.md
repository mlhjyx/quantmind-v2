...
### S8 — L4 STAGED 决策权 + DingTalk webhook 双向 ⭐⭐⭐ ⚠️ NEAR-COMPLETE (8a ✅ `dbf55c0`+sediment `dc17d88`; 8b ✅ PR #307 `e68b00a`+sediment `1442998`; 8c-partial ✅ PR #308 `3a4a324`+sediment 本commit; 8c-followup pending broker_qmt wire with explicit user ack)
...
| Acceptance progress | **8a ✅ DONE**: ExecutionPlan 不可变 dataclass + 状态机 + valid_transition 静态查表 + cancel_deadline ADR-027 §2.2 5 guardrails + STAGED_ENABLED=False default + DDL hypertable + 4 indexes + 180d retention + 39 tests. **8b ✅ DONE** (PR #307): 3-layer architecture + HMAC-SHA256 ±5min replay + constant-time compare + 4 verbs + race-safe UPDATE + LIKE wildcard escape + asyncio.to_thread + idempotent 2xx + empty secret→503 + strict UTF-8 decode. 48 tests. **8c-partial ✅ DONE** (PR #308): Celery Beat sweep `risk-l4-sweep-1min` crontab `* 9-14 * * 1-5` Asia/Shanghai + race-safe atomic UPDATE pattern (沿用 8b) + SWEEP_BATCH_LIMIT via settings override + STAGED smoke via RiskBacktestAdapter stub (0 broker / 0 alert / 0 INSERT) + adapter isolation verify pair. 25 tests (sweep 14 + smoke 11). **8c-followup PENDING** (deferred for red-line user ack): broker_qmt sell wire post-CONFIRMED + broker_order_id writeback + broker_fill_status + paper-mode integration smoke with real qmt_data_service |
| File delta | 8a: 4 files / 880 insertions. 8b: 8 files / 1768 insertions + reviewer follow-up. 8c-partial: 5 files / 691 insertions + reviewer follow-up. 8c-followup est: ~3-5 files / ~400-700 lines (with explicit user ack). |
| Chunked sub-PR | **chunked into 4**: 8a ✅ / 8b ✅ PR #307 / 8c-partial ✅ PR #308 / 8c-followup pending. Red-line decomposition pattern 1st 实证 — split scope to honor red-line gates without losing momentum (sustained pattern for .env paper→live cutover / broker_qmt wire / production yaml mutation). |
| Cycle | V3 §12.1 line 1317: 1-2 周 (range, replan 1.5x = 1.5-3 周). actual 8a+8b+8c-partial all <1 day each. 8c-followup TBD pending user ack. |
| Dependency | 前置: S6 ✅ / broker_qmt (现 PR #212 真 SQL 写第一次破除 sustained) / 后置: S9 |
| LL/ADR | ADR-027 (design SSOT) + ADR-056 (8a impl) + ADR-057 (8b impl) + **ADR-058 NEW** (8c-partial impl + red-line partial-decomposition pattern). LL-150 (8a) + LL-151 (8b) + **LL-152 NEW** (8c-partial sediment + 反 deepseek-style sediment gap 2nd 实证 sustained as enforcement). |
| Reviewer reverse risk | broker_qmt sell 单红线触发 (8c-followup ONLY) — quantmind-redline-guardian subagent 必 invoke; integration-first override 风险 (sustained S5/8a); 反 silent .env 改 live trading flag. **8a/8b/8c-partial 反 deepseek-style sediment gap**: 3 consecutive PRs (#307 + #308 + sediment) proactively wrote ADR + LL + REGISTRY + Plan amend in same session, reviewer agent invoked BEFORE merge, findings addressed BEFORE merge. 5-sprint cumulative deepseek-pattern lesson 现 sustained as enforcement. |
| 红线 SOP | redline_pretool_block hook + quantmind-redline-guardian subagent (双层); broker_qmt sell 单 (8c-followup) → STOP + push user; 8a/8b/8c-partial 0 broker call + 0 真账户 mutation + 0 .env mutation sustained. |
| Paper-mode | LIVE_TRADING_DISABLED=true sustained, broker_qmt sell 单 (8c-followup) paper-mode only |

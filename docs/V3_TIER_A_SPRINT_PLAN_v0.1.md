...
| element | content |
|---|---|
| Scope | V3 §11.1 row 5 `AnnouncementProcessor` (`backend/app/services/news/`); 公告流 巨潮/交易所 RSS; **post-Finding #2 (b) sediment** — Tier A 11 → 12 sprint. **Status (post sub-PR 11a sediment)**: 架构 sediment ✅ DONE (ADR-049: 6 architecture decisions + 3 findings resolution + chunked 2 sub-PR split) + DDL sediment ✅ DONE (announcement_raw NEW + rollback pair); implementation deferred to sub-PR 11b (待 user explicit ack). 沿用 ADR-049 V3 §S2.5 architecture sediment + RSSHub route reuse decision |

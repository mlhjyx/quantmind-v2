...
- Thursday 4-30 16:30
- Friday 5-01 16:30 (注意: 5-01 法定节假日, schtask 自动 skip)
- Monday 5-04 16:30 (5-02/5-03 weekend skip)

= **5 个 trade_day 真生产 STRICT OK** 后启动 Stage 3.0 PR.

## 生产 callers 实测 (Session 40 grep)

| 调用 | 位置 | 用途 | Stage 3.0 影响 |
|---|---|---|---|
| **L528** | `scripts/run_paper_trading.py` `run_signal_phase` | **signal_phase 写信号到 signals 表** | ⚠️ **本 PR 改造 target** |
| L631 | `scripts/run_paper_trading.py` `run_execute_phase` | execute_phase 读 latest signals (`get_latest_signals`) | ✅ 不动 (读路径, signals 表 schema 不变) |
...
- 1 周生产观察: 5-05 ~ 5-12 STRICT 仍 enabled, dual-run 仍跑 (Stage 3.0 wrapper 内调 SDK + legacy 比对) → 任 1 天 DIFF 立即回滚

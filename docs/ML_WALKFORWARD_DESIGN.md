...
> 仍有价值: WF 框架设计、fold 划分策略(F1-F7)、OOS 验证方法论 — 融合E2E (V4 Phase 2.1) 将复用此 WF 框架; LightGBM §1-§8 作为融合系统的层1训练框架继续使用
> 已过时/被替代: LightGBM 作为**独立策略**无效(排名→Top-N等权丢失alpha); 但其预测能力(IC=0.067)保留为融合系统的预测层
> V4 方向: 融合架构(LightGBM预测层 + Portfolio Network权重层) 章节见本文档末尾 §12 + QUANTMIND_FACTOR_UPGRADE_PLAN_V4.md 附录A
...
> - v2.2 (2026-04-10): LightGBM独立策略CLOSED → 融合架构预测层 + E2E章节追加 (V4 Phase 2方向)

---

## 十二、融合E2E架构 — V4 Phase 2 主攻方向 (2026-04-10)

> **本章定位**: 概述融合架构(LightGBM+Portfolio Network)与本文档§1-§11的关系。完整设计见 `QUANTMIND_FACTOR_UPGRADE_PLAN_V4.md 附录A`。

### 12.1 融合架构概览

| 维度 | LightGBM WF（§1-§8） | 融合E2E（V4附录A） |
|------|---------------------|-------------------|
| 角色 | ~~独立策略~~ → **E2E预测层** | 预测层+权重层融合系统 |
| 训练目标 | MSE on excess return | 层1: MSE / 层2: -Sharpe |
| 输出 | 预测收益→rank→Top-N | LightGBM得分→Portfolio Network→连续权重 |
| 状态 | **作为层1继续使用** | 层2待实现(V4 Phase 2.1) |
| WF框架 | ✅ 本文档 F1-F7 fold + walk_forward.py | ✅ 复用同一 WF 框架 |

### 12.2 LightGBM在融合架构中的角色

LightGBM **不是**被关闭的旧方法，而是E2E融合系统的**预测层**：
- LightGBM的IC=0.067信号有价值，丢弃=浪费
- 问题在portfolio构建层（排名→Top-N等权丢失alpha），不在预测层
- 融合架构：LightGBM(预测层) + Portfolio Network(权重层, loss=-Sharpe)

本文档§1-§8的LightGBM WF pipeline继续作为**层1训练框架**使用。
仅§8.3的"排名→Top-N等权→SimBroker回测"评估流程被E2E权重层替代。

### 12.3 WF框架复用方式

融合E2E训练将复用本文档定义的 Walk-Forward 框架:
- **Fold 划分**: 沿用 F1-F7 (§5), 训练窗口3年/验证1年/测试1年
- **OOS 拼接**: 沿用 chain-link Sharpe 方法 (§6)
- **差异**: 每个fold内先训练LightGBM(MSE)，再冻结LightGBM训练Portfolio Network(Sharpe loss)
- **对比验证**: 融合OOS Sharpe vs 等权OOS Sharpe (0.6336) vs LightGBM独立OOS Sharpe (0.09)

### 12.4 LightGBM独立策略结论

**LightGBM作为独立策略（predict→rank→Top-N等权）已确认无效：**
...
**结论：LightGBM不再作为独立策略，但其预测能力保留为融合系统的层1。**

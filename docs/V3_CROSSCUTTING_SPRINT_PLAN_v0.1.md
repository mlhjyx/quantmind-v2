...
> **✅ CLOSED 2026-05-14 (HC-1c 标注, ADR-073)** — chunked **5 sub-PR** (planned 3, +2 via precondition-surfaced scope splits, sustained ADR-022 反 retroactive content edit — 仅 append 标注, 下方 "chunked 3 sub-PR" row 保留): HC-1a PR #350 (Engine PURE interface + 5 元告警 rule 纯函数 + 38 tests) + HC-1b PR #351 (meta_monitor_service Application + meta_monitor_tasks Beat wire + runbook; 2 real + 3 no-signal collectors) + HC-1b2 PR #352 (channel fallback chain 主 DingTalk → 备 email → 极端 log-P0 + email_alert.py + 7 SMTP config 字段) + HC-1b3 PR #353 (DingTalk-push-status alert_dedup +2 cols + News per-source DataPipeline run-stats → Redis instrumentation) + HC-1c (本, ADR-073 + closure sediment, docs-only 直 push per 铁律 42). **元监控 alert-on-alert 4/5 collectors real** (LiteLLM/STAGED/DingTalk/News); **L1-heartbeat DEFERRED** (HC-1b3 Finding — 无 production XtQuantTickSubscriber runner, instrument 永不触发; 留 Plan v0.4 cutover scope 候选, ADR-073 D3). HC-1 scope balloon (3→5 sub-PR) = **Constitution §L0.4 replan-trigger surfaced + user-acknowledged** (每 scope fork 走 AskUserQuestion, Plan v0.3 §F (iii)). Gate D item 1 formal verify 留 HC-4c. Next: HC-2 (user 显式 ack required, sustained LL-098 X10).
...
> **✅ CLOSED 2026-05-14 (HC-2c 标注, ADR-074)** — chunked **5 sub-PR** (planned 3, +2 via HC-2a 12-item gap list 比 plan "wire any missing path" 假设大, sustained ADR-022 反 retroactive content edit — 仅 append 标注, 下方 "chunked 3 sub-PR" row 保留): HC-2a (V3 §14 15-mode enforcement matrix audit doc + 12-item gap list, bundled into PR #354) + HC-2b PR #354 (G5 RiskReflector 失败 retry-once-skip + RISK_REFLECTOR_FAILED 元告警 / G6 LIVE_TRADING_DISABLED 双锁 startup gate) + HC-2b2 PR #355 (G7 broker plan stuck sweep + BROKER_PLAN_STUCK 元告警 P0 + Beat `*/5` / G8 Redis cooldown auto-reconnect, 恢复无需进程重启) + HC-2b3 PR #356+#357 (G3 `evaluate_pg_health` PG OOM polled meta-alert + G4 `evaluate_market_crisis` Crisis regime polled meta-alert + `market_indicators_query` feed de-stub; #357 = post-merge python+database reviewer findings followup) + HC-2c (本, 灾备演练 synthetic injection ≥1 round 29 tests 12 modes + sediment doc `docs/risk_reflections/disaster_drill/2026-05-14.md` + ADR-074 + LL-167 closure sediment, backend/tests + docs PR per 铁律 42). **灾备演练 round 1 = 8/12 ✅ PASS full chain + 4/12 🟡 PARTIAL carried-gap** (mode 2/11 no production realtime runner → Plan v0.4 / mode 7+10 spec ⚠️ P2 → future sprint; drill REVEALING gaps 是其目的, ADR-074 D4). HC-2 scope balloon (3→5 sub-PR) = **Constitution §L0.4 replan-trigger surfaced + user-acknowledged — 2nd consecutive 横切层 sprint to balloon** (HC-1 also 3→5; 每 scope fork 走 AskUserQuestion, Plan v0.3 §F (iii)). Gate D item 2 formal verify 留 HC-4c. Next: HC-3 (user 显式 ack required, sustained LL-098 X10).
...
> **✅ CLOSED 2026-05-14 (HC-3b 标注, ADR-075)** — chunked **2 sub-PR (planned 2, actual 2 — 0 balloon**, 反 HC-1 3→5 + HC-2 3→5 双 consecutive balloon; HC-3 estimate held because both items = verify/audit-heavy doc deliverables on already-closed infra, NOT net-new wiring — LL-168 lesson 1): HC-3a PR #359 `2716af4` (V3 §17.1 CI lint 5-component verify-only report + **CRLF Finding fix** — `.gitattributes` `*.sh`+`pre-commit` `eol=lf`, `test_llm_import_block_governance.py` 8/10 RED on main `0165915` → 10/10 GREEN; code-reviewer APPROVE) + HC-3b 本 (prompts/risk eval iteration ≥1 round **structural baseline eval** on 5 YAML — paper-mode 0-traffic → skill §4 cost/quality routing triggers cannot fire, sustained ADR-072 D2; 5/5 prompts structurally sound 0 defect; **V4-Flash/V4-Pro routing 决议 = 5 prompts sustained 起手 model per ADR-036, cost/quality re-eval DEFERRED to Gate E**; + ADR-075 + LL-168 closure sediment, docs-only 直 push 铁律 42). HC-3a Finding (CRLF) surfaced via verify-only audit → user 决议 A 内修 (AskUserQuestion 1 round). HC-3b eval scope = structural (user 决议 A, AskUserQuestion 1 round — live-LLM fixture run + paper-mode 5d shadow DEFERRED). Gate D item 3+4 formal verify 留 HC-4c. 横切层 = 3/4 sprint closed (HC-1+HC-2+HC-3 ✅). Next: HC-4 (user 显式 ack required, sustained LL-098 X10).

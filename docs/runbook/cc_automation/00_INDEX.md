...
| 05 | [`05_send_task_queue_routing_sop.md`](05_send_task_queue_routing_sop.md) | Celery `send_task` queue routing — manual dispatch 真**queue= explicit** sustained 沿用 Beat entry config 反 default routing key "celery" silent drop reverse case (sub-PR 8b-cadence-B Option (C) 5-07 22:00 catch) | 0 (Redis broker write only + 0 broker call) |
...
| 07 | [`07_registry_status_count_sop.md`](07_registry_status_count_sop.md) | ADR `docs/adr/REGISTRY.md` status distribution count automation — committed/reserved/gap count update 沿用 PR add row 体例, N×N drift cross-verify (REGISTRY + ADR-DRAFT + V3 + LL backlog) sustained ADR-022 反 silent overwrite (sub-PR 8b-cadence-A reviewer HIGH 5-07 sediment) | 0 (doc-only) |

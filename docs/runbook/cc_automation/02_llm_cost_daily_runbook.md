...
- 0 SQL DML (仅 SELECT 聚合查询, INSERT 走 BudgetAwareRouter 真生产 path)
...
- (4) webhook_url 0 set 时** noop noop**, 反 push (沿用决议 (I))
- (5) ALERTS_ENABLED=False 时**反 push** (沿用 .env 双锁体例)

---

## 执行步骤

### Step 1: 本地 verify dry-run (0 push)
...
期望输出:
- exit 0 合法 (含 0 calls 月初空真值)
- markdown payload 完整 print (title + sum + breakdown)
- DB conn 连 quantmind_v2 + 0 SQL fail
...
### Step 3: push verify (DINGTALK_ALERTS_ENABLED=true 时)
...
期望:
- DingTalk **markdown push** 入群 (含 cost / breakdown / state label)
- 群可见消息 + errcode=0 (沿用 dispatchers/dingtalk.py 返)

### Step 4: schtask manual register (沿用 X10 + LL-098)
...
期望:
- Section 16 Register 成功 + Write-Host "[OK] QuantMind_LLMCostDaily registered"
- Get-ScheduledTask 返 State=Ready
- 实测下次 NextRunTime **Mon-Fri 20:30** (含跨周末跳过)
...
| # | 项 | ok 标准 | fail 处置 |
|---|---|---|---|
| 1 | --no-dingtalk verbose 生成 markdown | exit 0 + markdown payload 完整 | 检查 PG conn / SQL syntax / migration apply |
| 2 | DingTalk push errcode=0 (生产真测) | 群消息可见 + dispatchers log "成功" | 检查 webhook_url + secret + keyword 真值 |
| 3 | schtask Mon-Fri 20:30 NextRunTime 真值 | NextRunTime **Mon-Fri 20:30** | 重跑 setup_task_scheduler.ps1 + 实 Get-ScheduledTask |
| 4 | 1 周后 schtask 触发 (历史 LastRunTime) | LastRunTime **Mon-Fri 20:30 内** | 反 schtask Disabled / 反真生产 silent skip |
| 5 | llm_call_log 真生产含 row (BudgetAwareRouter wire 后) | row count > 0 | 检查 BudgetAwareRouter wire (audit param 传) |

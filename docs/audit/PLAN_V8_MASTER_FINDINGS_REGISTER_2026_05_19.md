...
|---|---|---|---|
| P0-2 | PG password plaintext + 6 .bak leaks | ✅ **PLAYBOOK READY** 5-20 (this batch): `docs/runbook/pg_password_rotate_playbook.md` sediment — pre-flight, rotation steps, post-flight verify, rollback procedure. User trigger required (high-risk credential mutation). Recommended Phase B-2 evening (5-27 Wed) post live flip. | YES |
...
| P0-4 | Reflector → ThresholdEngine NOT wired | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P0_4_REFLECTOR_THRESHOLD_ENGINE_WIRE_DESIGN.md` — 5 phase plan (schema → API → wire → backtest → auto-eval), human-in-loop initial pattern, ~2 weeks to closed-loop. Deferred Phase J post 5-27. | YES |
| P0-5 | Beat death no heartbeat (LL-181 sustained) | ✅ **CLOSED** 5-19 (this batch): scripts/audit_beat_heartbeat.py — external probe on celerybeat-schedule.dat mtime (robust to internal Beat deadlock, 反 LL-181). 300s stale threshold (conservative). Cold run 5-19 22:14 SH: BEAT-OK age=25.3s. DingTalk alert P0 on DEAD/MISSING. Schtask register 留 user (suggest /SC MINUTE /MO 5). | NO |
| P0-6 | Schtask freshness probe absent | ✅ **CLOSED** 5-19 (this batch): scripts/audit_schtask_freshness.py implemented + verified. PowerShell Get-ScheduledTaskInfo + classify(OK/FAIL/STALE/DISABLED) + JSON output + DingTalk alert. 5-19 cold run: 27 tasks (14 OK / 4 FAIL / 6 STALE / 3 DISABLED). Schtask register 留 user. | NO |
...
| P0-8 | Redis production data plane dark | ✅ **CLOSED** 5-20 (this batch): Redis ping PONG / 8 active qm:* streams (signal:generated / execution:order_filled / health:check_result / qmt:status / order:routed / news:last_run_stats / ai:monitoring / quality:alert) / portfolio:current HASH 0 fields (expected since 0 positions Phase B-1). Data plane NOT dark — production ACTIVE. | NO |
| P0-9 | 30 service-layer commit() violations | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P0_9_SERVICE_LAYER_COMMIT_REFACTOR_DESIGN.md` — Strategy B incremental per-service migration (Tier 3 utility → Tier 2 cold → Tier 1 hot), ~10-15 weeks. Phase J Week 2+ first action (post Phase B-2 stable). | YES |
| P0-10 | 铁律 18 slippage quarterly 0 scheduler | ✅ **CLOSED** 5-19 (this batch): Beat entry pre-existing (commit c1909d1, beat_schedule.py:~390). 本批 add `backend/app/tasks/slippage_calibration_tasks.py:quarterly_recalibrate` Celery task wrapper (subprocess invokes scripts/bayesian_slippage_calibration.py 23KB). Import verified + script exists. Beat fire 5-20 不会再 ModuleNotFoundError. | NO |
...
| P0-12 | 11 pytest collection errors | ✅ **CLOSED** 5-19 (this batch): root cause = .venv missing `tenacity` + `feedparser` (pyproject.toml declarations sustained, env unsynced). `pip install tenacity feedparser` 真 apply. Re-verify: 6251 tests collected (5999→6251, +252 tests), 0 errors. **Prior "natural closure" claim (commit 7ddc3f1) 实际 stale**. | NO |
| P0-15 | 09:30 SH market open no watcher | ✅ **CLOSED** 5-19 (this batch): scripts/audit_market_open_watcher.py — 3-check probe (trading_day DB SSOT + weekday heuristic fallback / market_hours within 9:30-15:30 SH / Beat alive via celerybeat-schedule.dat mtime). DingTalk P0 alert on ALERT severity. 5-19 22:45 cold run verify: is_trading_day=True / market_hours=False (expected post-market) / Beat=fresh. Exit=1. Schtask register 留 user (suggest /SC DAILY /ST 09:31). | NO |
| P0-16 | No monthly LLM cost audit aggregator | ✅ **CLOSED** 5-19 (this batch): Script `scripts/llm_cost_monthly_audit.py` pre-existing (5-19 20:06 SH, ~7.7KB). Beat entry pre-existing (commit c1909d1, beat_schedule.py:382). 本批 add `backend/app/tasks/llm_cost_audit_tasks.py:monthly_audit` Celery task wrapper (subprocess invokes script). Import verified + script exists. Beat fire 5-20 不会再 ModuleNotFoundError. | NO |
| P0-17 | HC-2c disaster drill pytest-only | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P0_17_DISASTER_DRILL_DESIGN.md` — 3 tier scenario catalog (12 scenarios) + Phase 1 simulation script + quarterly cadence (§VIII #29 integration). 2 gaps surfaced: disk_full + LIVE_TRADING_DISABLED runtime guard. Deferred Phase J post 5-27. | YES |
...
| P0-21 | Live trade reproducibility 4 sources stale | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P0_21_LIVE_TRADE_REPRODUCIBILITY_DESIGN.md` — 4 source capture (signal context / execution decision / market state / config snapshot), 3 new tables, replay API. 6 phase plan ~3-5 weeks. Phase J Week 1-3 priority post 5-27 (critical: capture wire MUST precede any incident). | YES |
...
| P1-26 | FundamentalContextService 1/8 维 | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_26_FUNDAMENTAL_CONTEXT_8_DIM_DESIGN.md` — 7 missing dims (industry macro / shareholder / operating quality / governance / valuation / capital flow / announcement) + aggregator pattern. 5 phase plan ~7-9 weeks. Phase J Week 4+ start with Dim 6+7 (估值 + 资金流) high priority. |
...
| P1-31 | Loop 1 Signal 0 NAV feedback | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_31_32_CLOSED_LOOP_DESIGN.md` §2 Loop 1 design — read-only Phase 1 + soft-adapt Phase 2 + hard-adapt Phase 3+, 4 phase plan ~2 months to closed-loop. Bounded ±25% factor weight adjustment. Deferred Phase K. |
| P1-32 | Loop 4 Regime → Signal disconnect | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_31_32_CLOSED_LOOP_DESIGN.md` §3 Loop 4 design — Option B continuous weight modulation (bounded ±20%), 5 phase plan ~2 months to production. Deferred Phase K. |
| P1-33 | SPF 6 no Tushare fallback chain | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_33_TUSHARE_FALLBACK_CHAIN_DESIGN.md` — Tushare primary → Baostock fallback (P1) → Akshare tertiary (defer). 4 phase plan ~2-3 weeks to Phase 3, quarterly reconciliation audit ongoing. Deferred Phase J post 5-27 PT live. |
...
| P1-36 | Risk subservice cluster 0 API | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_36_RISK_SUBSERVICE_API_DESIGN.md` — 7+ subservices, read-only API Phase 1 (7 endpoints) + write API Phase 2 (4 endpoints) + debug Phase 3, 4 phase plan ~2-4 weeks. Deferred Phase J post 5-27. |
| P1-38 | DEV_AI_EVOLUTION Layer 3+4 0% impl | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_38_DEV_AI_LAYER_3_4_DESIGN.md` — Layer 3 (self-improvement) Phase K ~6 months, Layer 4 (cross-strategy) Phase L ~12-18 months. Defer both (pre-requisites: 6 months stable live data + ≥2 strategies). |
...
| P1-47 | Frontend dual chart libs ECharts+Recharts | ✅ **DESIGN SEDIMENTED** 5-20 (this batch): `docs/design/P1_47_FRONTEND_CHART_LIB_MIGRATION.md` — Consolidate to ECharts (financial domain SSOT), migrate 3 Recharts pages (AssistPanel + PT_Dashboard + SystemSettings), 4 phase plan ~4-5 days. Deferred Frontend Phase H W7+. |

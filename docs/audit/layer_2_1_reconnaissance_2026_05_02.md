...
|---|---|---|---|---|
| `klines_daily` | 2026-04-28 | 11,776,616 | 4-29/4-30 真是 trading_day (`trading_calendar`实测), klines 4-29=0 + 4-30=0 真没拉 | ❌ **真 stale (pull pipeline silent stop, NOT holiday)** |
| `daily_basic` | 2026-04-28 | 11,681,799 | 同 klines (4-29/4-30 真没拉) | ❌ **同 klines 真因 (pull pipeline silent stop)** |
| `factor_values` | 2026-04-28 | 840,478,083 | 依赖 klines, klines stale → factor stale | ⚠️ 依赖 klines |
| `factor_ic_history` | 2026-04-28 | 145,894 | 同 klines (Layer 1 fast_ic_recompute MAX cite 一致 ✅) | ⚠️ 依赖 klines |
| `moneyflow_daily` | **2026-04-30** | 11,458,587 | 4-29=5144 / 4-30=5142 真拉了 (反 klines/daily_basic 同根假设, 真值: pull pipeline 不同根) | ✅ 真新 |
...
→ 真因 (反原 framing 修订): `data_fresh` check 走 `trading_calendar` 表查 prev_trading_day (`scripts/health_check.py:39-66`), today=5-02 → prev_trading_day=4-30 (真值, 4-30 真是 trading_day). klines MAX=4-28 < 4-30 → **真正确报警**, **NOT holiday tolerance 误报**. 真因: klines 4-29 + 4-30 真没拉 (per-date COUNT=0 实测), pull pipeline silent stop. moneyflow_daily 4-29=5144 / 4-30=5142 真拉了 — 反 schtask 同根假设, 真因 pull pipeline 不同根.
...
→ 真因 (反原 framing 修订): **同 health_check 同根, 但根因 NOT holiday tolerance**. P0 alert 真正确触发 (klines/daily_basic 4-29 + 4-30 真没拉, 真 stale 2 trading days). DingTalk push 真发 (alert key=`data_quality:summary:2026-04-30`). 真因: pull pipeline silent stop (见 §I 2.1.7 reconnaissance prompt).
...
| **F-RECON-2** ⚠️ **framing v2 修订** (Path B sediment 2026-05-02): klines/daily_basic 4-29 + 4-30 真没拉 (4-29/4-30 真是 trading_day per `trading_calendar` 实测, klines/daily_basic per-date COUNT=0). 反 v1 framing "holiday-driven 节后自然恢复". moneyflow_daily 4-29=5144 / 4-30=5142 真拉了 → 反 schtask 同根假设, pull pipeline 真不同根. | **独立 (NOT holiday-driven, 节后不会自然恢复)**: pull pipeline silent stop, 真因待诊断 (见 §I 2.1.7) |
...
| ~~**2.1.3**~~ | ~~health_check + data_quality_check holiday-aware tolerance 升级~~ | ~~scope 取消 (Path B 决议 2026-05-02)~~ | — | — | ❌ **CANCELED** (反原 framing: health_check 真已走 trading_calendar 真 holiday-aware, schtask LastResult=1 真因不是 tolerance, 是 klines/daily_basic 4-29/4-30 真没拉 — 见 2.1.7) |
...
| **2.1.7** ⭐ **NEW** | klines/daily_basic pull pipeline 4-29+4-30 silent stop 真因诊断 | 真测 pull schtask 真状态 + script 真实现 + 4-29 user 决议链 (`SHUTDOWN_NOTICE_2026_04_30` / `link_paused_2026_04_29`) 是否真 disable klines/daily_basic schtask + 候选 root cause 拆分 (silent fail / disabled / stop). reconnaissance only, 0 修代码. | Layer 2.1 audit 接续 | 0 真金风险 (read-only) | **P1** (真生产数据 stale, schtask 持续真报警, 节后不会自然恢复) |

### E.1 推荐起手次序 (Path B 修订 2026-05-02)

1. **并列首推**:
   - **2.1.1** T0-19 audit chain 4-29 历史 backfill 决议 + verifier 修 (4-29 真生产事件, 真金 0 风险, PR #168 既有 hook 复用, verifier false-positive 是 Layer 1 Week 1 漏)
   - **2.1.7** ⭐ klines/daily_basic pull pipeline 4-29+4-30 silent stop 真因诊断 (节后不会自然恢复, schtask 真持续报警, reconnaissance only 0 修代码)
2. **次推 2.1.2** (minute_bars pipeline 诊断, read-only)
3. **2.1.6 留 Layer 2.2** (修代码 ≠ reconnaissance)
4. **2.1.4 / 2.1.5 留 Layer 2.3** (cleanup, 非紧急)
5. **2.1.3 已 CANCELED** (原 framing 错, 真因走 2.1.7)

### E.2 不推荐 / scope 守门

- ❌ **不推荐**: 一起性把 2.1.1 ~ 2.1.7 全做 — 反 sub-task 串行交付铁律 23/24, 留 user 决议起 2.1.1 / 2.1.7 后再 plan 下一个
- ❌ **不推荐**: 直接修 klines/daily_basic pull schtask / backfill 4-29+4-30 历史数据 — 真因未诊断 (2.1.7 reconnaissance), 盲修 risk PT 真生产 (虽然 PT 已 disabled, 数据 backfill 仍可能影响 factor_values 历史)
...
3. 数据源 stale 3 独立 finding chain: trade_log gap (PT 暂停) / **klines/daily_basic 4-28 真因 = pull pipeline 4-29+4-30 silent stop (NOT holiday-driven, framing v2 修订)** / minute_bars 4-13 (~13 trading days, pipeline 真不存在)
...
**推荐起手 (Path B 修订)**: 并列首推 2.1.1 (T0-19 audit chain backfill + verifier 修) 与 2.1.7 (klines/daily_basic pull pipeline silent stop 真因诊断). 两者 0 真金风险 + reconnaissance only.

**user 决议起**: 待 user 真决议 sub-task 2.1.1 / 2.1.7 起手 / 改其他 / 跳过 / 调整 scope.

> **Framing v2 修订记录** (2026-05-02 Path B): 原 §C.1 / §C.2 / §D.1 F-RECON-2 / §E 2.1.3 / §H #3 标 "klines stale = holiday-driven" 真错. 真测 `trading_calendar` + per-date COUNT 实测 4-29/4-30 真是 trading_day, klines/daily_basic 真没拉 (moneyflow 同期真拉). 反 anti-pattern v5.2 第 7 次实证守门生效 — STOP + 反问 user 触发, sediment 进 §I 2.1.7 reconnaissance prompt + §J reviewer kill incident.

---

## §I Sub-task 2.1.7 reconnaissance prompt sediment

**编号**: Layer 2.1.7
**title**: klines/daily_basic pull pipeline 4-29+4-30 silent stop 真因诊断
**优先级**: P1 (并列 2.1.1 首推)
**scope**: reconnaissance only, 0 修代码 / 0 backfill / 0 改 schtask

### I.1 背景 (反 anti-pattern v5.2 第 7 次实证)

Layer 2.1 reconnaissance §C.1/§C.2/§D.1 v1 framing 错: "klines stale = holiday-driven 节后自然恢复". 真测真值 (Path B 决议时 实测):

| 数据源 | 4-27 | 4-28 | 4-29 | 4-30 | trading_calendar |
|---|---|---|---|---|---|
| klines_daily | 5481 | 5474 | **0** | **0** | 4-29/4-30 = True |
| daily_basic | 5481 | 5474 | **0** | **0** | 4-29/4-30 = True |
| moneyflow_daily | 5177 | 5170 | **5144** | **5142** | 4-29/4-30 = True |

→ klines/daily_basic 4-29 + 4-30 真没拉, 但 moneyflow 同期真拉了 → schtask 真不同根. **真因不是 holiday tolerance**.

### I.2 真测目标

A. **klines/daily_basic pull schtask 真状态**
   - 真测 schtasks 真名 (cite memory 候选: `QuantMind_DailyKlines` / `QM-DailyMoneyflow` 类似但 klines 真名未实测)
   - LastResult / NextRun / Disabled / Last Run 时序 (4-28 / 4-29 / 4-30 / 5-01)
   - 对比 moneyflow_daily 真拉 schtask 配置差异

B. **pull script 真生产路径**
   - grep `pull_klines|fetch_klines|tushare.*daily|update_klines_daily` 真生产 entry
   - 真测 4-28 → 4-29 时序: schtask 真触发 / script 真启动 / Tushare API 真 call / DB INSERT 真发生 哪一环 silent stop
...
D. **候选 root cause + 候选 action**
   - 候选 RC1: schtask 真 disabled (4-29 user 决议链顺手关)
   - 候选 RC2: schtask 真活但 script silent fail (Tushare auth / network / DB lock)
   - 候选 RC3: schtask 真活但 script 真触发, Tushare 4-29/4-30 真没数据 (反 trading_calendar, 不太可能)
   - 候选 action A: backfill 4-29/4-30 历史数据 (单次手工拉, 0 改代码)
   - 候选 action B: 修 schtask 真活 (取决 RC1/RC2)
   - 候选 action C: 接受 stale 待 PT 重启时再恢复

### I.3 STOP 触发

- 真测发现真生产红线异常 (LIVE_TRADING_DISABLED / EXECUTION_MODE) → 立即 STOP
- 真测发现 4-29 user 决议链真包含 disable klines/daily_basic, 但 PT 重启 gate prerequisite (`SHUTDOWN_NOTICE §9`) 真包含 enable 之 → 报告状态, 0 改动 (留 user 决议)
- 真测发现 silent fail 真因涉及多 schtask / 多 script / 多 caller → STOP + 反问扩 scope

### I.4 不做

- ❌ 修任何代码 / 改 schtask 配置 (.ps1 注册 / Disabled 状态)
- ❌ backfill 4-29/4-30 数据 (留 sub-task 2.1.7 后续决议)
- ❌ 跑 PT / 重启 / 改 .env

### I.5 验收

- audit doc (新 file `docs/audit/klines_pull_silent_stop_2026_05_xx.md`)
- A-D 全 4 块真测真值 + 命令 + evidence
- 候选 root cause + 候选 action 推荐
- 0 修代码 / 0 数据改动
- branch push, 0 PR (本 sub-task 仍 reconnaissance)

---

## §J Reviewer kill incident sediment (LL candidate)

### J.1 incident 真值 (2026-05-02 ~03:01)

PR #203 reviewer agent (oh-my-claudecode:code-reviewer) launched async background review. **status=killed** mid-flight.

**Partial findings before kill** (favorable):
- §A emergency_close log 13992 B exact match ✅
- §B f19 backfill line PASS ✅
- §B `insert_trade` dead code claim correct ✅
- §C minute_bars F-RECON-3 grep PASS ✅
- 0 P0 / 0 P1 in partial scope

**Reviewer 未达** internal consistency / arithmetic check / §D F-RECON-2 framing 真值 verify (本 incident 后 STOP+反问 user 触发).

### J.2 真因候选 (未深查)

- 候选 1: agent timeout 真值 ≤ 30 min (audit doc 真大 + reviewer 多 grep 多 read 真耗时)
- 候选 2: parent thread 真发 STOP 信号 (CC 自身 STOP+反问 触发 agent kill)
- 候选 3: dev infra 真异常 (oh-my-claudecode harness / async agent runtime)

### J.3 LL 候选 sediment

**LL-100 候选** (待 user 决议是否 promote):
- **trigger**: agent reviewer kill mid-flight (status=killed) 后, 不硬 push 接受 partial review
- **rule**: kill incident 后, 评估 partial findings 完整度 (覆盖 §A/§B/§C/§D 多少). 若 partial 0 P0/P1 + 覆盖 ≥ 50% scope → 可接受 partial; 否则 retry 1 次 (chunked review or extended timeout). retry 仍 kill → STOP + 反问 user.
- **rationale**: 反 "reviewer kill 后硬 push" anti-pattern. partial review ≠ 完整 review, 但完整 review 0 进展更糟.

### J.4 本 PR (#203) reviewer retry 决议

Path B Task 3 真决议 (2026-05-02 ~03:30):
- retry 1 次 chunked reviewer (single agent, scope 限制为 §D F-RECON-2 framing 修 verify + §I 2.1.7 sediment 合理性 verify, 反 v1 整 doc review 真大耗时)
- retry kill → 接受 partial v1 review (已 0 P0/P1 + 覆盖 §A/§B/§C 真值) + sediment 本 §J → AI self-merge

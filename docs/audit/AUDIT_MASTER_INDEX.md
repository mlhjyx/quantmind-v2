...
| Phase C C2 | F31 拆分 milestone 2 — load_* → factor_repository + PEAD 纯化 | ⬜ 待开始 | — | — | — |
...
- ⏳ **下一 session (C2)**: 搬家 `load_daily_data` / `load_forward_returns` / `load_bulk_data` / `load_bulk_moneyflow` / `load_index_returns` / `load_bulk_data_with_extras` / `load_fundamental_pit_data` → 新 `backend/app/services/factor_repository.py`, 同时把 `calc_pead_q1` 拆成 `load_pead_announcements(conn, ...)` + 纯计算 `calc_pead_q1(ann_df)`. 验收: `test_pead_factor` 全绿 + 金标 max_diff=0.

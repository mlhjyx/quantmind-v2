...
| DCM-015 | P1 | Pipeline status could be blocked indefinitely by historical `pipeline_runs.status='running'` rows, and the GP weekly path failed because the DEAP dependency was not declared/installed. | Runtime `/api/pipeline/status` exposed stale runs `gp_2026w16_000147e8` and `gp_2026w15_000147e8`; latest terminal row then showed `DEAP未安装`. | Fixed 2026-05-29 follow-up: status surfaces `is_stale_running` / `stale_reason`, localhost-only cancel endpoint + UI button closes stale rows explicitly, the two stale rows were cancelled, and `deap>=1.4.1` is now a project dependency. Remaining evidence: next scheduled GP first-fire should prove a non-DEAP failure/closure path. |

...
| Backtest platform | MOSTLY_IMPLEMENTED | `PlatformBacktestRunner`, CLI integration, `/api/backtest/run` Celery enqueue, and the API worker execution path are now unified on the Platform runner. Remaining partial scope is non-archive research-script migration/triage plus controlled runtime evidence. |
...
| BacktestRunner is the unified entry. | `backend/qm_platform/backtest/runner.py`, `scripts/run_backtest.py`; API submit path enqueues `app.tasks.backtest_tasks.run_backtest`, whose worker delegates execution through `PlatformBacktestRunner` before writing the existing API result tables. | `backend/tests/test_backtest_runner.py`; `backend/tests/test_backtest_api.py`; `backend/tests/test_backtest_tasks.py`; `frontend/src/__tests__/backtest-api.test.ts`. | MOSTLY_IMPLEMENTED | Research-script migration/triage and controlled API worker first-fire evidence remain B1 follow-up. |
...
|---|---|---|
| B1 | Backtest execution closure | **MOSTLY DONE 2026-05-29 follow-up.** API submit has tested Celery enqueue and UI payload alignment; the Celery worker now executes via `PlatformBacktestRunner` with `InMemoryBacktestRegistry` while preserving the existing API result persistence contract. Remaining acceptance: triage non-archive research scripts still bypassing the platform runner and collect controlled API worker first-fire evidence. |

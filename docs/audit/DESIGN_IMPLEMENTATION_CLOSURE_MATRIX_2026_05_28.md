...
| GP/mining closed loop | PARTIAL_IMPLEMENT | GP engine/orchestrator/script/API exist. BruteForce API/task is no longer a placeholder and writes `bf_` quick-gate candidates with pending full-review metadata. Mining full-gate wrapper now uses the real `FactorGatePipeline.run_gates` contract. Automatic scheduling plus approval feedback into next-round seed/blacklist are not closed. |
...
| GP closed loop is complete. | GP engine/orchestrator/script/API exist; BruteForce task now reaches executable engine logic; full-gate wrapper uses the current GateReport contract. | GP/mining tests exist, including BruteForce task runner and full-gate contract coverage. | PARTIAL_IMPLEMENT | B3. |

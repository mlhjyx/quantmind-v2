...
| GP/mining closed loop | PARTIAL_IMPLEMENT | GP engine/orchestrator/script/API exist. BruteForce API/task is no longer a placeholder and writes `bf_` quick-gate candidates with pending full-review metadata. Automatic scheduling plus approval feedback into next-round seed/blacklist are not closed. |
...
| GP closed loop is complete. | GP engine/orchestrator/script/API exist; BruteForce task now reaches executable engine logic. | GP/mining tests exist, including BruteForce task runner coverage. | PARTIAL_IMPLEMENT | B3. |

...
>
> **本文件版本**: v0.8 (post-V3 Tier B Plan v0.1 sub-PR sediment + plan-then-execute 体例 第 4 case Tier B context + §2.2 Tier B sprint chain row expansion T1.5+TB-1~5 per Plan v0.2 5 决议 lock, 2026-05-13, sub-PR Plan v0.2 sediment cycle — Plan v0.2 NEW + Constitution v0.8 → v0.9 + ADR-064 NEW + LL-158 NEW + REGISTRY ADR-064 row + memory handoff Session 53+1, 沿用 ADR-022 反 silent overwrite — v0.1-v0.7 row 保留 + version history append + §2.2 sustained S12-S15 abstract row + new T1.5+TB-1~5 Plan v0.2 mapping row 体例 sustained sub-PR 8 sediment cumulative scope 第 4 case Tier B context)

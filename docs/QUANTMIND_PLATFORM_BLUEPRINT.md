# QuantMind Platform Blueprint (QPB v1.11)

> **本文件**: QuantMind V2 平台化蓝图 — 从"脚本堆"到"Core Platform + Applications"的演进规划
> **创建**: 2026-04-17 v1.0 → v1.1 (+#11 ROF + U6) → v1.2 (4 主决策) → v1.3 P0 补丁 → v1.4 Cold Start Ready → v1.5 Wave 2 Data 层完结 → v1.6 Wave 3 MVP 重排 — MVP 3.1 Risk Framework 新增 (ADR-010 PMS Deprecation) → v1.7 Session 24-27 收束 — MVP 3.1 批 0 feasibility spike ✅ + ADR-010 addendum (方案 C Hybrid adapter) + 铁律 43 登记 → v1.8 Session 28-30 收束 — MVP 3.1 Risk Framework 正式完结 (6 PR) → v1.9 (intermediate stamp) → v1.10 Session 38-39 (2026-04-27 Monday 18:00-22:30) — MVP 3.3 Signal-Exec 60% 完成 + LL-081 三通道闭合 + LL-076 phase 2 真完结 (单日 10 PR / Wave 3 进 3.x) → **v1.11 Session 39 加时 (2026-04-28 00:00-01:00 Tuesday 凌晨) — MVP 3.3 batch 2 Step 2 + 2.5 双完结 (PR #110/#111/#112) + STRICT flip 真切换 LIVE (Windows User env via setx)**
> **作者**: Architect pass (Opus)
> **状态**: v1.11 (12 Framework + 6 升维 + **17 MVP**, 27-36.5 周), **Wave 1 ✅ 完结 7/7 + Wave 2 ✅ 完结 + Wave 3 1.7/5 完结 (MVP 3.1 ✅ + MVP 3.2 ✅ + MVP 3.3 75% — batch 1/2.1/2.2/3 ✅ + Step 2.5 STRICT flip live, batch 2.3 = Stage 3.0 真切换待 Session 40+)**
>
> **v1.11 验证证据** (gh 实测 2026-04-28 01:00 Session 39 加时末): MVP 3.3 batch 2 merged PR = **#110 (Step 2 SDK dual-run helper warn-only) + #111 (Step 2.5 STRICT mode env-gated raise + `SignalPathDriftError(RuntimeError)`) + #112 (sdk_parity_scan.py multi-date pre-flip 验证 + STRICT flip executed via setx)**. 14/14 历史 trade_dates (4-07~4-24) bit-identical pre-flip 验证 (max_w_diff=0.00e+00 全场), STRICT flip live (Windows User env `SDK_PARITY_STRICT=true`, schtask 16:30 自动继承). LL-085 (历史多日 batch > 等 production 单数据点) + LL-086 (Windows setx schtask env propagation, 不需 Servy restart) 入册. 0 P1 / 14 P2 + 10 P3 全采纳跨 3 PR. **下 session 真主线**: Stage 3.0 真切换 (signal_service.write_signals 走 SDK Order, 删 legacy compose), 解锁条件 = 1 周连续 STRICT OK (Tuesday 16:30 ~ next Monday 5 production days).
> **参考**:

...
**P0 触发 (2026-05-06 user finding)**: "CLAUDE.md / IRONLAWS.md / LESSONS_LEARNED.md / SYSTEM_STATUS.md 4 doc 整 session 反 fresh read sustained, CC 显然没有触发这些, 这是为什么?"

**真根因**: SOP gap — Sprint 2 sub-PR 1-6 累计沉淀 仅 外 source fresh verify (智谱/Tavily/Anspire/GDELT/Marketaux/RSSHub docs, 沿用 LL-104 cross-verify 体例), 反 sediment 内 source (4 root doc) fresh read SOP. 致 5-02→5-06 累计 ~3-4x 真值漂移:
...
| 4 doc mtime | "5-06 sustained" | **0/4 5-06** (CLAUDE/IRONLAWS=5-01, LESSONS/SYSTEM_STATUS=5-03) | 真因 |
...
2. **LL-106** — 5-06 P0 finding sediment + memory cite 5-02 沉淀真值漂移 ~3-4x cite source 锁定真值 + PR-A 真讽刺 sediment 实证 #4 cite source 锁定真值. ll_unique_ids canonical 97 → 98.
...
**(1) LL-only sediment 反铁律 (governance 弱)** — 仅 LESSONS_LEARNED.md +LL-106 sediment, 不加铁律. **未选**: LL backref governance 弱 (CC 自主自律), pre-commit hook 仅 warning, 反 block. 历史已证 LL-103 真分离 architecture finding 类 sediment 30 min 后即被违反 (沿用 LL-103 SOP-4 真讽刺案例第 1 次). 反复实证 governance 单层 (LL-only) 不足.

**(2) 沿用 LL-119 SOP only (现状, 反 sediment 内 source)** — 不引 SESSION_PROTOCOL.md / 铁律 45 / LL-106, 沿用 sub-PR 1-6 体例. **未选**: 5-06 P0 finding 真因即 SOP gap, 反 sediment 内 source 直延续致后续 sub-PR 起手时累计漂移更深 (next 7-N).

**(3) 仅 SESSION_PROTOCOL.md create (PR-A only) 反铁律 + ADR + LL** — sediment 实操 SOP 但反 governance enforcement. **未选**: SOP 文件 0 enforcement 触发机制 (CC 自主读 vs 强制), 沿用 LL-098 X10 governance pattern 类比, X10 当时即沉淀为铁律 (反仅 LL-098), 本 SOP 沿用同体例 sustained.

## Consequences

**正面**:
- Governance 双层防御 sustained (铁律 45 enforcement + SOP file detailed 体例 + LL-106 cite source 锁定真值)
...
- 真讽刺自身实证 #4 真生产 captured (PR-A SOP 首版含 phantom LL → reviewer fix → 完整闭环, sediment cite source 锁定真值)
- 跨 session resume 时强制 fresh verify 4 doc, 反 5-02→5-06 类似漂移累计

**负面**:
- sub-PR / sub-step 起手前 4 步 fresh read 真时间成本 (~1-2 min/sub-PR, 沿用 PR-A fresh verify 实测)
- IRONLAWS rule count 增 1 (45 → 46 rules), 文档引用漂移候选 (各 audit / handoff cite "45 rules" / "T1=31" 真值漂移 audit Week 2 batch sediment 候选 sustained)
...
- create [docs/SESSION_PROTOCOL.md](../SESSION_PROTOCOL.md) (160 行 / reviewer findings 全采纳 sustained)
...
- create [LESSONS_LEARNED.md](../../LESSONS_LEARNED.md) +LL-106 真新建 (next free, ll_unique_ids canonical 97 → 98)
...
- SYSTEM_STATUS.md update — sub-PR 6 RSSHub Servy register service count drift (4 → 5 sustained)
...
- [ADR-021](ADR-021-ironlaws-v3-refactor.md) — 铁律 v3.0 重构 + IRONLAWS.md 拆分 + X10 加入 (本 ADR 沿用同体例 sustained)
...
- LL-067 — Reviewer agent 是 AI 自循环 PR 流程的真正第二把尺子 (PR-A 真讽刺案例 sediment 真生效 cite source)
...
- LL-103 — Claude.ai vs CC 真分离 architecture (governance 双层防御实证)
- LL-104 — Claude.ai 写 prompt 时表格 cite 仅看 1 row 真不够, 必 grep 全表 cross-verify
- LL-105 — ADR # reservation 真预约 4 source cross-verify SOP-6 sediment
- PR #237 — PR-A SESSION_PROTOCOL.md create (本 ADR 真讽刺案例 #4 source)

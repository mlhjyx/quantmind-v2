...
- **真生产代码 SSOT** (router.py + types.py): BULL_AGENT + BEAR_AGENT mapping V4-Flash sustained PR #221 sediment
- **V3§5.5 mapping table 文档 cite** (line 660/661/724): "V4-Flash" sustained 5-02 sediment ✅ align 真生产代码
- **V3§11.2 service cite** (line 1228, MarketRegimeService): "LiteLLMRouter (V4-Pro)" — **internal drift sustained 5-02 sediment**
- **V3§19 cost 估算** (line 1589): "V4-Flash | 180 calls/月 | $0.3/月" — align V3§5.5 mapping (V4-Flash)

**5-06 user 决议**: BULL/BEAR mapping V4-Flash → V4-Pro (debate agent 真**reasoning capability** 需求, 反 V4-Flash 轻量分类). 沿用 V3§11.2 line 1228 service cite 已 V4-Pro 体例修复 V3 internal drift.
...
|---|---|---|
| RiskTaskType.NEWS_CLASSIFY | "deepseek-v4-flash" | sustained |
| RiskTaskType.FUNDAMENTAL_SUMMARIZE | "deepseek-v4-flash" | sustained |
| **RiskTaskType.BULL_AGENT** | **"deepseek-v4-flash"** | **"deepseek-v4-pro"** |
| **RiskTaskType.BEAR_AGENT** | **"deepseek-v4-flash"** | **"deepseek-v4-pro"** |
| RiskTaskType.EMBEDDING | "deepseek-v4-flash" | sustained |
| RiskTaskType.JUDGE | "deepseek-v4-pro" | sustained |
| RiskTaskType.RISK_REFLECTOR | "deepseek-v4-pro" | sustained |
...
| V3§11.2 line 1228 (MarketRegimeService) | "LiteLLMRouter (V4-Pro)" | sustained ✅ (本 PR align target) |
...
| 候选 | 描述 | 评价/理由 |
|---|---|---|
| (1) 沿用 V4-Flash | 5-02 sediment | ❌ 拒 — debate agent 真 reasoning capability 需求, V4-Flash 轻量分类反足. V3§11.2 service cite 已 V4-Pro, internal drift sustained. |
...
- **debate reasoning capability 提升**: V4-Pro (deepseek-v4-pro = R1 reasoning) 真 nuanced reasoning capability, 反 V4-Flash (deepseek-v4-chat 轻量分类). Bull/Bear 论据 / Judge 加权决策走 V4-Pro 沿用 reasoning 体例 (V3§5.5 + ADR-028 §2 反思 reasoning quality 体例延伸).
- **V3§5.5 align V3§11.2 修复 internal drift**: V3§11.2 line 1228 service cite 已 V4-Pro sustained 5-02 sediment, V3§5.5 mapping table sustained V4-Flash sustained drift. 本 PR 沿用 V3§11.2 修订 V3§5.5, 修复 V3 internal drift.
...
- **BudgetGuard CAPPED_100 触发概率沿用 0 影响**: cost 远低 cap, 反触 CAPPED_100. PR #223 sediment sustained.

### Neutral

- **router.py PRIMARY_MODEL_SUBSTRINGS sustained 0 改**: alias `deepseek-v4-flash` + `deepseek-v4-pro` resolve substring 沿用 PR #221 sediment.
- **LiteLLM yaml 0 改**: model_list alias sustained, 反加新 alias (反 V4-Mid 候选). yaml fallback chain sustained PR #221 体例.
- **Sprint S12 (V3 line 1329) cite "L2 Bull/Bear 2-Agent debate (V4-Flash + V4-Pro Judge)" sustained 0 patch**: Sprint 名 cite 沿用 5-02 sediment, 反 mapping detail. 走 audit Week 2 batch sediment 候选 (LL drift 候选).
...
| Step 7 | test mock re-run verify (48 mock + 2 e2e + smoke 55 sustained) | CC (本 PR) | 5-06 ✅ |
...
- V3§11.2 (Service contract, MarketRegimeService line 1228) — sustained V4-Pro, 本 PR align target
...
- 5-06 Sprint 2 prerequisite Step 2.5 4 source cross-verify (V3 internal drift sustained finding)

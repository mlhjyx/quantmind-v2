...
| 3 | Adapter pattern | ⚠️ **AMENDED sub-PR 13 (ADR-052 reverse)**: ~~RSSHub route reuse~~ → **AKShare direct API** | **真值 verified broken via sub-PR 13 Phase 0 active discovery** (LL-142): local RSSHub `/cninfo/announcement/*` HTTP 404 (5 variants probed) + upstream rsshub.app HTTP 403 production block + local instance lacks cninfo namespace plugin. **Reverse decision per ADR-052**: switch to `AkshareCninfoFetcher` (NEW) using `ak.stock_zh_a_disclosure_report_cninfo` — real-data 真测 1:1 simulation 2026-05-09 fetched=10 real cninfo announcements for 600519. Original Decision 3 cite sustained for cite trail (ADR-022 反 silent overwrite), 沿用 sub-PR 13 sediment for真值 reverse evidence. |

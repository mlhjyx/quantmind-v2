...
**SSOT**: [docs/adr/REGISTRY.md](REGISTRY.md) (本 # 状态)

---

## §1 Context

V3 Sprint 1 S2 sub-task 起手前,需决议 LiteLLMRouter implementation path。

现状:
- `backend/engines/mining/deepseek_client.py` (396 lines) + 2 callers (factor_agent.py / idea_agent.py) + 57 mock 测试,Sprint 1.17 sediment GP 闭环 prototype
- 该模块含 ModelRouter (4 路由: IDEA/FACTOR/EVAL/DIAGNOSIS) + CostTracker + DeepSeekClient
- V3 §5.5 待办 LiteLLMRouter (新模块) 服务 risk control 域 6 任务 (NewsClassifier / summarizer / Bull-Bear / Judge / RiskReflector / Embedding + Ollama fallback)
...
1. **ADR-020** (V3 §18.1 row 2 待办): LiteLLM 是 only path,沿用 4-29 user 决议
...
- 长期不 deprecate 违反 V3 §5.5 unified path 精神
...
├─ 路径: backend/qm_platform/llm/router.py (V3 §5.5 待办)
...
3. ✅ `grep "DeepSeekClient\|ModelRouter"` 0 production 输出 (cross-verify SSOT)
...
- 渐进 deprecate path 0 生产中断 (0 hot path)
- ADR-020 + ADR-022 + V3 §5.5 待办 三联齐
...
- 长期债务: 若 deprecate 未 sediment (e.g. caller 切换 PR 永远不起手),violator 长期保留 (S6 hook allowlist marker 月度 audit 机制 §9 防御)

### §5.3 Neutral

- ModelRouter 4 路由 → 渐进 deprecate 后,GP 因子挖掘域全统一走 LiteLLMRouter LLM 路由
...
- [x] **本 PR**: ADR-031 sediment + S8 audit + REGISTRY.md update + V3 §18.1 row 11 待办
...
- [x] **Sprint 2 sub-PR 7b.2** (#241, 5-07): NewsClassifierService L0.2 V4-Flash sediment (caller class + yaml prompt + tests mock-only)
 - 实现: `backend/app/services/news/news_classifier_service.py` (V3 line 1223 待办 path) + `prompts/risk/news_classifier_v1.yaml` (V3 line 390 待办 yaml)
 - 路由: `RiskTaskType.NEWS_CLASSIFY` → `deepseek-v4-flash` (router.py:57 sediment, ADR-035 §2 V4 路由层 0 智谱)
 - 灾备: Ollama qwen3.5:9b (LLMResponse.is_fallback → ClassificationResult.classifier_cost=NULL, ADR-031 §6 灾备体例)
 - 测试: 47 mock-only (4 category × 4 profile × 4 urgency × sentiment boundary + parse fail-loud + persist NotImplementedError verify)
 - persist hook stub = NotImplementedError 沿用本 §6 line 141 "Sprint 2-N caller 切换" 体例 留 sub-PR 7b.3 wire
- [x] **Sprint 2 sub-PR 7b.3 v2** (#242, 5-07): NewsClassifierService.persist wire + bootstrap factory + requires_litellm_e2e marker register + e2e live test
 ...
 - 铁律: 32 (Service 不 commit, caller 管事务) + 31 (DataPipeline 0 DB IO) + 33 (news_id=None fail-loud)
 ...
 - drift catch case #10 + #11 候选 lesson 应用: sub-PR 7b.3 v1 STOP push back (CC fresh verify) → v2 (β) split sediment 反 pipeline.py 架构违反 + 反 phantom marker reference
- [x] **Sprint 2 sub-PR 7c** (#243, 5-07): NewsIngestionService orchestrator + Sprint 2 ingestion 闭环 Layer 2.2 完整闭环 sediment ✅
 - 实现: `backend/app/services/news/news_ingestion_service.py` (V3 line 1222 待办 path, NewsIngestionService class + IngestionStats dataclass) + `__init__.py` +exports
 - 全链 architecture (沿用铁律 31 + 32 drift catch case #11 候选 lesson 应用):
 - DataPipeline (sub-PR 7a #239, qm_platform/news/, 0 DB IO 铁律 31) → list[NewsItem]
 - 本 service (app/services/news/, orchestrator **入库点**) → conn → INSERT news_raw RETURNING news_id (9 cols, NewsItem 1:1 align)
 ...
 - 0 conn.commit (铁律 32, caller 事务边界管理者)
 - per-item fail-soft (沿用 sub-PR 7b.2 contract): ClassificationParseError → audit log + skip + classify_failed count, news_raw row **已 INSERT 成功** (容忍 partial classify, sub-PR 7b.2 **未来 backfill** path 待办)
...
- [docs/audit/sprint_1/s8_deepseek_audit.md](../audit/sprint_1/s8_deepseek_audit.md) — 本 ADR audit 数据来源
- [docs/QUANTMIND_RISK_FRAMEWORK_V3_DESIGN.md](../QUANTMIND_RISK_FRAMEWORK_V3_DESIGN.md) §5.5 (LLM 路由待办) + §16.2 (cost monitoring) + §18.1 row 2 (ADR-020 reserve) + row 11 (本 ADR row,本 PR 新增)
- [docs/adr/REGISTRY.md](REGISTRY.md) — 本 # 状态 SSOT (本 PR 新增 row)
- [docs/LLM_IMPORT_POLICY.md](../LLM_IMPORT_POLICY.md) §8 — Known Legacy Violator (deepseek_client.py:222 marker 待办 sediment)

...
- **V4 路由层 ADR-031 §6**: V4-Flash + V4-Pro = DeepSeek (deepseek-v4-flash + deepseek-v4-pro), Ollama qwen3.5:9b 灾备 (PR #228 sediment). **0 智谱 alias**.
...
- ADR-031 §6 (S2 LiteLLMRouter implementation path): V4 路由层 DeepSeek + Ollama, 0 智谱 alias
...
| GLM-4.6v paid 6M (visual, 7-13 到期) | 反 V3 Tier A scope (Tier B+ 候选) | 0 caller |
| search-std/pro 100 次 | 反 MCP 沿用, 0 caller (反 LLM 路由层) | 0 wire |
| 通用 tokens 2M / 按次 20 次 | 沿用 fallback 候选 (audit Week 2 batch) | reserve |

### V4 路由层 0 智谱 (沿用 ADR-031)
...
| 候选 | 描述 | 评价/理由 |
|---|---|---|
| (1) 沿用 5-02 ADR-033 GLM-4-Flash cite | 反修订 | ❌ 拒 — 0 GLM-4-Flash model 实测, 5-02 cite drift 反 silent overwrite (ADR-022) |
| (2) 智谱走 V4-Pro fallback (GLM-4.7 paid 5M) | 沿用 5-02 (b) cite | ❌ 拒 — V4 路由层 ADR-031 DeepSeek + Ollama, 智谱走 News ingestion 层独立 client |
...
| **(4) GLM-4.7-Flash News#1 + V4 路由层 0 智谱 (本 ADR 采纳)** | 5-06 user (a) + (b) 取消 | ✅ 采纳 — 永久免费 / specs 优 / V4 路由层 ADR-031 / 0 商授 risk |
...
- **V4 路由层 ADR-031**: DeepSeek + Ollama 灾备体例, 0 智谱 alias 反 LiteLLM yaml 改 (Step 2.5 finding).
...
- **search-std/pro 0 wire 0 sediment**: 智谱独立产品 reserve, 反 V3 §5.5 路由层 cite, 反 V3 Tier A scope (Tier B+ 候选).
...
- **GLM-4.7-Flash free tier qps cap finding** (5-06 Step 4-3 smoke test 实测): 4 candidate model_id fresh smoke test 实测 — `glm-4.7-flash` 反 HTTP 429 "访问量过大" rate limit code 1305 (free tier qps cap, 反 monthly token quota only), `glm-4.5-flash` / `glm-4.5-air` / `glm-4.7` 全 HTTP 200 ✅ 0.6-2.6s. Sprint 2 implementation 起手时走 LiteLLM provider timeout=30s + retry backoff 沿用 PR #221 体例反 free tier qps spike 反 caller fail (沿用 cold-start 60s retry sediment Step 4-2 体例).

### Neutral

- **5-02 ADR-033 sediment**: ADR-033 patch 修订智谱 model name + Anspire 申请方式 + Marketaux 范围 + RSSHub 中文财经源 verify, 沿用 §1 SSOT drift 主动 finding 体例.
- **sprint_state v7 line 110/119/197/240/265 老 6 源 cite drift**: 走 memory direct write patch (反 git PR scope), 沿用 v7 patch 体例 (PR #227 sediment).
- **API key 申请方式修订**: Anspire 5-06 web_search 实测 = 即时 (反 5-02 cite "审批 1-2 周"), 沿用 LL-116 候选 sediment.
- **真生产 cost ~$0.10/月** (沿用 Step 2.5 cite): News#1 GLM-4.7-Flash $0 + V4 路由层 ~$0.10/月 + Ollama $0. 远低 V3§20.1 #6 $50 cap.
...
- LL-098 X10 (反 forward-progress default) — 本 PR 0 user 接触 implementation
- LL-114 候选 (cite drift cross-source) — sprint_state v7 老 6 源 cite drift
...
- 5-06 user 决议 (a) GLM-4.7-Flash News#1 + (b) GLM-4.7 paid V4-Pro fallback 取消
- 5-06 web_search Step 2 + Step 2.5 cite (智谱 + DeepSeek + 4 source cross-verify)
- 5-06 Step 4-3 fresh verify (4 candidate model_id smoke test: `glm-4.7-flash` HTTP 429 / `glm-4.5-flash` + `glm-4.5-air` + `glm-4.7` HTTP 200 + docs.bigmodel.cn fresh fetch — GLM-4.7-Flash 200K context + 128K output, GLM-4.5-Flash deprecating soon)
- 5-06 user 决议 Step 4-3: 沿用 (2) GLM-4.7-Flash ADR-035 §2 (反 (1) 改 4.5-Flash / 反 (3) 双 model fallback / 反 (4) 别)

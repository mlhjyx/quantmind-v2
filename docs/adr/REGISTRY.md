...
| ADR-037 | Internal source fresh read SOP — 4 root doc + sub-PR/sub-step 起手前必走 enforcement (governance) | committed | V3 Tier A Sprint 2 prerequisite Step 4-7 v2 PR-B (本 PR), 沿用 PR-A #237 SESSION_PROTOCOL.md create + 5-06 user P0 finding sediment + 真讽刺案例 #4 PR-A SOP 首版含 phantom LL-119/115 sediment cite source 锁定真值. 沿用 ADR-022 集中修订机制 + ADR-021 X10 governance pattern. 关联铁律 45 (新, T1 tier) + LL-106 (新, ll_unique_ids 97→98) |
| ADR-039 | LLM audit failure path resilience — retry policy + transient/permanent classifier (S2.4 sub-task partial closure) | committed | V3 Tier A Sprint 2 sub-PR 8b-llm-audit-S2.4 (本 PR), 沿用 ADR-DRAFT row 7 promote + ADR-031 §6 sub-task partial closure + sub-PR 8a-followup-B-audit PR #248 BUG #2/#3 sediment + sub-PR 8b-llm-fix PR #253 production primary path 真生效. 真**残余 sub-task**: circuit breaker (sub-PR 8b-resilience 真预约) + DingTalk push (sub-PR 9 真预约) sustained. 沿用 LL-067 reviewer 第二把尺子 + LL-098 X10 sequence-based + LL-100 chunked SOP. **跳号 ADR-038 sustained** (LiteLLM cost registry V4 gap 真**reserve** ADR-DRAFT row 6 promote target, 等 LiteLLM SDK 升级 verify v4-* registry 真生效时 sediment) |
| ADR-040 | (informally reserved, ADR-DRAFT row 8) DeepSeek API watch SOP + 7-24 deadline plan governance | reserved | ADR-DRAFT row 8 informal reservation (audit Week 2 batch B 候选), 沿用 SOP-6 cross-verify 体例 sustained, audit chunk C ADR # 重整时 promote 候选 |
| ADR-041 | (informally reserved, ADR-DRAFT row 9) yaml double-model alias-underlying sync 体例 governance | reserved | ADR-DRAFT row 9 informal reservation (governance + 7-24 deadline plan migration governance PR), 沿用 SOP-6 cross-verify 体例 sustained, audit chunk C ADR # 重整时 promote 候选 |
...
> **新 committed**: ADR-037 (5-06 Step 4-7 v2 PR-B sediment) — Internal source fresh read SOP (governance) — 4 root doc + sub-PR/sub-step 起手前必走 enforcement. 沿用 PR-A #237 SESSION_PROTOCOL.md create + 真讽刺案例 #4 sediment + 5-06 user P0 finding cite source 锁定真值. 关联铁律 45 (新, T1) + LL-106 (新, ll_unique_ids 97→98).

...
| 13 | D1 O3 pipeline pause gate (`POST /api/pipeline/pause` + `/resume` + extend pipeline_settings singleton) | iter 12 Inner-loop-B sediment 2026-05-23/24 (L4+R autonomous loop, /loop self-paced): design `docs/design/PN_003_pipeline_pause_gate.md` 完成. frontend `pipeline.ts:122 pausePipeline()` 已 wired 但 backend 0 endpoint (API_COVERAGE §6 O3 → 404); PipelineConsole.tsx:206 graceful catch. §7 重蹈 defense PASS (0 permanent-dead / conditional-fail; LL hits 都是 Beat schedule paused 7 天 / PR #150 link-pause 不同 scope). §6 8-trigger STOP self-check 全 NEGATIVE (Feature 级 — orchestration gate-at-entry, 0 mid-run abort / 0 broker / 0 .env). §4.5 value verdict IMPLEMENT (复用 PN-001 singleton + PN-002 mock-test 体例; storage: `paused_at TIMESTAMPTZ NULL` + `paused_reason TEXT NULL` ALTER ADD COLUMN; gate at `POST /trigger` (409 Conflict) + `run_gp_mining` / `run_bruteforce_mining` task body entry (skip + structured log); resume idempotent NULL set; 6 backend pytest + 2 frontend vitest). | candidate | sub-PR iter 12 implement step promote |

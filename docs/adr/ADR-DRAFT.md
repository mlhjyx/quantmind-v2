...
| 2 | News fetch query strategy SSOT (5 源 vs RSSHub route path 体例分裂) | sub-PR 8a 5-07 sediment, V3§3.1 sub-PR 6 docstring "RSSHub 走独立 pipeline" sustained. **5-07 sub-PR 8b-cadence-A partial promote**: Beat schedule mechanism (Celery Beat 反 Task Scheduler) + cron 频率 (`crontab(hour="3,7,11,15,19,23", minute=0)` 4-hour offset 3h) + RSSHub 路由层契约 (standalone POST /api/news/ingest_rsshub) + cost cap decision sediment (沿用 row 6 真预约) + dingtalk rate cap decision sediment (沿用 sub-PR 9 alert wiring 真预约) + Risk v2 Beat indefinite paused 体例 sustained 真**ADR-043** sediment ✅; 真**残余 sub-task**: News Beat schedule entry register (sub-PR 8b-cadence-B 真预约) + RSSHub multi-route 503 fix (audit chunk C 真预约) sustained | **→ ADR-043 (committed, partial)** | 沿用 sub-PR 8b-cadence-B + audit chunk C 真预约 完整 closure |

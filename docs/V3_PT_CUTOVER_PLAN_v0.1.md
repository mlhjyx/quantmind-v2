...
| element | content |
|---|---|
| Scope | **(SHUTDOWN_NOTICE §9 prereq cleanup)** DB 4-28 stale snapshot 清理 — DELETE 19-stock stale `position_snapshot` rows + `cb_state` reset to live=¥993,520 (per SHUTDOWN_NOTICE §9 prereq, user 显式触发 SQL DELETE — redline-guardian 双层); DB row mutation 是 CT-1 期 ONLY mutation type. **Paper-mode V3-in-path shake-down**: ~1-2 自然日 paper-trading run 走 fully-integrated V3 chain (NOT legacy) — operational shake-down to catch UI/log/alert flow issues that replay can't catch (real DingTalk push / real PG load / real Servy resource use); 5 SLA (V3 §13.1) measured during shake-down with real traffic patterns. |

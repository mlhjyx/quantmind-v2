# AGENTS.md — QuantMind V2

> **入口文件** (Codex 启动自动读取). 只含编码必需信息.
...
> **重构沿革**: [ADR-021](docs/adr/ADR-021-ironlaws-v3-refactor.md) (铁律 v3.0) + [Step 6.3b STATUS_REPORT](docs/audit/STATUS_REPORT_2026_05_01_step6_3b.md) (AGENTS.md Path C 精简)
...
- **PMS**: v1.0 (pms_engine + api/pms + Beat task) **物理退役 iter 50 2026-05-24** (ADR-094). V3 风控保留 V3 §4 L1 PMSRule 纯规则实现 (`backend/qm_platform/risk/rules/pms.py`) 作回归/复用锚点；14:30 `risk-daily-check` Beat 已退役, 当前生产化方向由 L1 RealtimeRiskEngine tick/5min path + V3 §7.3 trailing_stop 承接.
- **下一步**: **🎯 Wave 4 100% COMPLETE — 4.1 + 4.2 + 4.3 + 4.4 全 ✅** (iter 51-75 单日 cumulative 2026-05-25: MVP 4.1 Observability 17/17 + MVP 4.2 Performance Attribution 7/7 + MVP 4.3 CI/CD 7/7 + MVP 4.4 Backup & DR 7/7, 总计 ~226 cumulative unit tests + 2 NEW Beat entries (daily-backup-run 02:30 / weekly-backup-verify Sun 04:00) + 5 NEW backup orchestrators + 5 NEW ci/ orchestrators + 1 daily-attribution-compute Beat + .github/workflows/ci.yml). 主线 transition to **post-Wave-4 audit-driven phase (iter 100-132)** — iter 100-129 cluster (factor_lifecycle P0 + W14 frontend + conftest 12/12) + iter 130-132 Audit Week 2 (W2-A 6 finding catalog 3 P0 closed iter 131 / iter 132 PR #484 Wave 4 audit envelope meta_monitor+attribution, reviewer cycle 1 P0 pending §v9.39 fix-flow). **Tier A Phase J 5 chain backlog sustained** (L1 RealtimeRiskEngine 0 production caller / L4 STAGED planner 0 caller / 流 5 daily_reconciliation 设计+wire backend-only ✅ closed iter 164-168 MVP 4.6 (PRs #500/#501/#502) / 流 6 RAG consumer ✅ closed iter 173-178 MVP 4.7 (PRs #503/#504/#505/#506/#507) / 流 3→4 event publish ✅ closed iter 183-185 MVP 4.8 (PR #510) — **Phase J 5-chain 100% backend-only ✅, runtime-verified pending Servy unblock per LL-210 iter 169** / 流 3→4 event publish 5min polling gap; 详 [PHASE_J_DEFER_MANIFEST](docs/audit/PHASE_J_DEFER_MANIFEST_2026_05_20.md)). **Tier B Wave 5 Operator UI = 5/5 sub-MVPs ✅ 100% MILESTONE (iter 196-216 cumulative 21-iter span)**: MVP 5.1 PT 状态 (iter 196-199) + MVP 5.2 IC 监控 (iter 201-204) + MVP 5.3 回测对比 (iter 205-208) + MVP 5.5 调度 dashboard (iter 209-212) + MVP 5.4 风控事件追踪 (iter 213-216). 11 PRs merged (#511-#521), ~3500 LOC frontend + ~600 LOC backend + ~40 TDD tests, 10 AI reviewer cycles sustained + ~8 production bugs prevented. Runtime-verified pending Servy unblock (sustained Tier A§5 gate, now flips 9 MVPs: Phase J 4 + Wave 5 5). **DEV_AI honest ~30-45%** (Layer 3-4 全 0% + strategy_agent MISSING + Orchestrator 0 中枢, per Session 58+1 5-19 addendum). PT 重启 gate prerequisite 见 [SHUTDOWN_NOTICE_2026_04_30 §9](docs/audit/SHUTDOWN_NOTICE_2026_04_30.md). 历史 V4 路线图 (Phase 1.1-3 + Phase 4) 全 ✅ 或 NO-GO 沉淀, 详 SYSTEM_STATUS.md §0.
...
| 步 | 文件 | 用途 |
|---|------|------|
| 1 | `AGENTS.md` (本文件) | 编码规则 + 铁律 tier 索引 + 文档地图 |
...
- **minute_bars**: 190,885,634 行 (~36 GB), 5年(2021-2025), Baostock 5分钟K线, 2537只股票(0/3/6开头, 无BJ) **[iter 52 2026-05-25 fresh verify 100% 匹配 Session 45 4-30 count; iter 169 fresh max_td=2026-04-13 — 注意 "since 4-30" 旧版表述指 PT 4-29 暂停 epoch (无新 Baostock pull), NOT data max_td (max_td 实际 4-13, iter 171 wording 订正); vs SYSTEM_STATUS.md:726 stale 139M 4-17 snapshot — 1.37x diff 是 SYSTEM_STATUS stale, NOT AGENTS.md 错误]**
...
```
quantmind-v2/
├── AGENTS.md / IRONLAWS.md / SYSTEM_STATUS.md / LESSONS_LEARNED.md / FACTOR_TEST_REGISTRY.md  # 根目录 5 doc (铁律 22 同步)
├── docs/
...
- **当前实施 (V3)**: V3 §4 L1 `PMSRule` (`backend/qm_platform/risk/rules/pms.py`) 作为纯规则与历史语义锚点保留；14:30 `risk-daily-check` Celery Beat 已退役. 当前生产化风险路径以 L1 RealtimeRiskEngine tick/5min 任务 + **V3 §7.3 `trailing_stop`** (`backend/qm_platform/risk/rules/realtime/trailing_stop.py`) 为准.
...
- **候选未 promote (Step 6.2.5+)**: X1 (Codex 边界) / X3 (.venv fail-loud) / X4 (死码月度 audit) / X5 (文档单源化)
...
22. [T2] 文档跟随代码 — AGENTS.md/SYSTEM_STATUS/DEV_*/Blueprint 同步或 `NO_DOC_IMPACT`
...
- **历史引用** (AGENTS.md inline 时代写的 "铁律 33"): 沿用编号, 仍可走本段 reference 找到 link
...
| 阶段 | 状态 | 关键产出 | 详情 |
|---|---|---|---|
| **Audit Week 2 + iter 130-132 governance** | 🟢 in flight (2026-05-26) | iter 130 AUDIT_WEEK2_MANIFEST + digest #12 / iter 131 W2-A 6 finding catalog (3 P0 closed + F6 V3 Regime LLM parse + F7 phantom import) / iter 132 PR #484 Wave 4 audit envelope meta_monitor+attribution (reviewer cycle 1 P0 pending §v9.39) / iter 133 hook silent-on-clean + AGENTS.md sync | L4R_AUDIT_WEEK2_MANIFEST + W2_A_OBSERVABILITY_MVP41_RUNTIME_VERIFY |
...
- **测试基线** (iter 232 fresh verify 2026-05-27, iter 237 ruff 0 errors + iter 241 flaky H2 mitigation, 铁律 40 baseline 保持): **6869 collected / 0 collection error** (vs iter 80 baseline 6714 = +155 / +2.3% Wave 5 iter 196-216 ~55 unit tests cumulative + sibling additions; vs Session 9 实测 2864 pass / 24 fail, ~1.5 月增量 +4005 tests / +140%) + **fail baseline iter 241 = 0 or 1 fail (TBD on next full sweep)** (iter 232 FIXED Windows GBK encoding bug; iter 241 FIXED test_factor_determinism H2 banker's-rounding straddle case via `.round(6)` removal per iter 238 §3 root cause analysis — H2 mitigation isolated PASS 2/2 in 58.74s; H1 cross-test DB row mutation still latent if real culprit, post-fix sweep would expose via diff>>1e-6 rather than silent flakiness) + **smoke 91 PASS** (iter 189 pre-push hook scope broadened backend/tests/smoke/ → backend/tests/, +30 smoke-marked tests previously ungated; sustained pre-push hook green iter 189-241 cumulative ~50 push cycles) + **ruff baseline iter 237 = 0 errors** (down from iter 236 17→2; remaining 2 UP042 StrEnum migration deferred via `# noqa: UP042` on BackupTarget + CIPhase pending 24-caller audit) + regression 5yr+12yr max_diff=0 (baseline 2026-04-28 stale, 待 Phase B-2 refresh)
...
### 根目录只允许以下文件
AGENTS.md / IRONLAWS.md / SYSTEM_STATUS.md / LESSONS_LEARNED.md / FACTOR_TEST_REGISTRY.md / pyproject.toml / .gitignore
...
### 数字同步规则
- AGENTS.md中的统计数字（表数/因子数/测试数）变更时同步更新
- 不确定的数字标注"约"或"截至日期"

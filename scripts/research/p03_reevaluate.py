...
all_factors = [
    ...
    "bp_ratio",  # CORE3
    "reversal_20",
    "amihud_20",  # CORE5 extras
    "RSQR_20",
    "dv_ttm",  # Phase 2.4 additions
]
...
for key, cfg in configs.items():
    ...
    results[key] = metrics
    print(
        f"    Sharpe={metrics['sharpe']:.4f}  MDD={metrics['mdd']:.4f}  AnnRet={metrics['annual_return']:.4f}  ({metrics['elapsed_s']}s)"
    )
...
for key, m in results.items():
    print(
        f"{m['label']:<35} {m['sharpe']:>8.4f} {m['mdd']:>8.4f} {m['annual_return']:>8.4f} {m['total_return']:>8.4f}"
    )

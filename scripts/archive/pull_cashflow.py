#!/usr/bin/env python3
"""拉取Tushare现金流量表(cashflow) -> cash_flow表。
...
关键规则:
  ...
  5. n_cashflow_act = 经营活动净现金流，accrual_anomaly因子核心字段

用法:
    python scripts/pull_cashflow.py                    # 全量拉取
    python scripts/pull_cashflow.py --offset 100       # 从第100只开始(断点续传)
    python scripts/pull_cashflow.py --verify           # 仅验证
"""
...
CF_FIELDS = [
    ...
    "lt_amort_deferred_exp", "invest_loss", "loss_fv_chg",
...
def fetch_cf_by_stock(ts_code: str, retry: int = 3) -> pd.DataFrame:
    """按股票拉取cashflow全部历史（合并报表）。
...
def process_cf_df(df: pd.DataFrame, symbols_set: set) -> pd.DataFrame:
    """清洗现金流量表数据。

    1. 去除代码后缀(.SZ/.SH -> 纯代码)
    2. 过滤无效symbols
    3. 日期转换
    4. PIT: f_ann_date为NULL时fallback到ann_date，最后fallback到end_date+90天
...
def upsert_cashflow(df: pd.DataFrame, conn) -> int:
    """批量upsert到cash_flow。"""
    if df.empty:
        return 0
    ...
    sql = f"""INSERT INTO cash_flow ({col_str})
              VALUES ({placeholders})
...
def verify_data(conn):
    ...
    # 总行数
    cur.execute("SELECT COUNT(*) FROM cash_flow")
    ...
    # 日期范围
    cur.execute("SELECT MIN(report_date), MAX(report_date) FROM cash_flow")
    r = cur.fetchone()
    print(f"  报告期范围: {r[0]} ~ {r[1]}")

    # 覆盖股票数
    cur.execute("SELECT COUNT(DISTINCT code) FROM cash_flow")
    ...
    # n_cashflow_act非空率（accrual_anomaly核心字段）
    cur.execute("""SELECT COUNT(*) FILTER(WHERE n_cashflow_act IS NOT NULL)::float
                   / NULLIF(COUNT(*), 0)
                   FROM cash_flow""")
    r = cur.fetchone()[0]
    print(f"  n_cashflow_act非空率: {r:.2%}" if r else "  n_cashflow_act非空率: N/A")

    # n_cashflow_act范围抽样
    cur.execute("""SELECT report_date, MIN(n_cashflow_act), MAX(n_cashflow_act),
                          AVG(n_cashflow_act), COUNT(*)
                   FROM cash_flow
                   WHERE report_date >= '2023-01-01' AND n_cashflow_act IS NOT NULL
    ...
    # free_cashflow非空率
    cur.execute("""SELECT COUNT(*) FILTER(WHERE free_cashflow IS NOT NULL)::float
                   / NULLIF(COUNT(*), 0)
                   FROM cash_flow""")
    r = cur.fetchone()[0]
    print(f"  free_cashflow非空率: {r:.2%}" if r else "  free_cashflow非空率: N/A")
    ...
    # 重复检测
    cur.execute("""SELECT code, report_date, COUNT(*) FROM cash_flow
...
def main():
    parser = argparse.ArgumentParser(description="拉取Tushare现金流量表数据")
    ...
    for i, (_code, ts_code) in enumerate(stocks):
        ...
        if df.empty:
            ...
            consecutive_fail = 0
            df_clean = process_cf_df(df, symbols_set)
            n = upsert_cashflow(df_clean, conn)
            total_rows += n

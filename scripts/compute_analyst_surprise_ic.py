...
def main():
    ...
    if abs(t_stat) > 1.96 and abs(ic_mean) > 0.02:
        ...
        print("  PEAD effect confirmed in A-share. Recommend for candidate pool.")
    elif abs(t_stat) > 1.64:
        print(f"  MARGINALLY significant (t={t_stat:.2f}, IC={ic_mean:.4f})")
        print("  PEAD signal present but weak. May improve with longer CAR window.")
    else:
        print(f"  NOT significant (t={t_stat:.2f}, IC={ic_mean:.4f})")
        print("  PEAD proxy via CAR not effective. May need analyst consensus data.")
    print(f"{'='*70}")

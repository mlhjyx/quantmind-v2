...
- 任 cross-system claim (user "跟 Codex 说过 X" / "Codex.ai 决议 Y" / "memory cite Z")
...
user "跟 Codex 说过 X" / "Codex.ai 决议 Y" / "memory cite Z" 等跨 system claim:
...
- Codex.ai vs CC 分离 architecture (LL-103 sediment) — conversation 不 cross-sync, 必 fresh verify
...
| v5 Codex 给具体 → CC 实测决议 | prompt 仅写方向 / 目标 / 验收, CC 实测 path / SQL / command |

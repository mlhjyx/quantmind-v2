...
| 类型 | 触发条件 | 处理 |
|---|---|---|
| **(a) "和我假设不同"** | prompt cite / Codex.ai cite / memory cite 跟 fresh verify 真值漂移 (任一数字 / 编号 / 存在 / mtime / cross-reference 类) | **立即 surface** (反 silent 沿用 prompt cite 推进) — cite 漂移类型 + 真值 + 修正 scope |
...
| 实证 | finding |
|---|---|
| PR #270 `.Codex/` audit (5-08) | 8 Codex.ai finding cross-verify, drift 率 25% (2 ❌ + 1 🟡 + 5 ✅), LL-116 主 sediment |

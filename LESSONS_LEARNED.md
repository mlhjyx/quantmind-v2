...
- 真金 ¥1M 0 熔断保护 (无 PMS L1/L2/L3 + 无 CircuitBreaker L1-L4 + 无 IntradayDrop)
...
- **双重检测** 防 zombie 假阳性:
  - Layer 1: 4 Servy 服务 `sc query` 状态 → 真死服务必抓
...
| 钉钉 dedup window | 1h | 防 96/日 × 钉钉无配额 spam, 真出事故 1 次/h 告警足够 |
...
PT_Watchdog 1/日 20:00 模式留 17h 静默 gap. 真金链路 (尤其 9:00 + 14:30 高 stake 时点) 必须 ≤ 15-30min 频次健康检查.

#### LL-074-C: zombie process 检测必须看应用层 artifact, 不能只看服务状态

`Get-Service Status=Running` 不等于 Beat 真在跑 schedule. PersistentScheduler 5min cycle 写 `celerybeat-schedule.dat` LastWriteTime 才是真心跳 (类似 LL-068 schtask hang 也是看 stderr/log freshness).
...
MVP 3.3 batch 3 (PR #109) `PlatformOrderRouter` 接 `audit_trail.record('order.routed', ...)` hook 时, **初版**写在 `orders.append(order)` 紧后 (per-signal 循环内). 审查发现: 当后续 `turnover_cap` 全局检查 raise `TurnoverCapExceeded`, 已生成的 1-N 个 Order 对应的 audit records 已 fire — Step 1 stub 仅 logger.info 无副作用, 但 MVP 3.4 替换为 `DBOutboxAuditTrail` 时, **outbox 表已写但实际 Order 未下单**, caller 重试整批 → outbox 重复 + 与真单不一致.

### 根因

audit hook 时机错配. 原意 "Order 创建即刻 record" 是同步直觉, 但忽略了**整批校验失败**场景下, 已 record 但未真生效的 phantom entries 污染 audit trail. 这类 contract 设计需考虑:
- **stub 期 vs concrete 期行为差异**: 当前 stub 安全, 替换 concrete (真写 DB / event_bus) 时延迟暴露
...
- LL-076 phase 2 真完结 2 PR (#104/#106)
...
- reviewer 发现的真 bug 包含 multiple 高价值 proactive 修 (LL-082 + LL-083)
...
- 时间紧 + 累 + regression 真硬门 = 高 fail 风险, 不强推
...
- **batch 切片设计**: SDK only / production switching 分开 PR, regression 真硬门隔离
- **reviewer agent 当 partner**: 不当 process gate, 信任 0 假报 + 真 bug 抓取能力
...
## LL-097: schedule / config 注释 ≠ 真停服 — 必显式 restart (X9, 批 2 P0 修, 2026-04-30 19:30+)

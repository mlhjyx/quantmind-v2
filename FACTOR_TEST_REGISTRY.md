...
> ⚠️ **口径说明** (Plan 3, 2026-05-20): 此 `M=88` 仅计本表 detailed 行数; §累积统计 (本文件顶部 L13) 的 `M=213` 额外含 128 Alpha158 批量因子 (未逐行登记)。BH-FDR 的 M 真值口径尚未统一 (严格视角 ≈240, 见 §累积统计「M 漂移评估」), 留 future 因子研究 PR 决议。CLAUDE.md §因子审批硬标准 引用的是 M=213 (§累积统计 SSOT)。本说明仅 surface 不一致, 不改任何 M 数值。

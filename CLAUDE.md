...
- 🟡 **Wave 2 进行中** (Session 8 末 2026-04-19): Data Framework 基础 + Data Lineage + MVP 2.1c 完整结案 + **MVP 2.3 Sub1 核心 3 PR + SDK 成熟扩 5/5 + 首个生产 script 迁 SDK ✅**. **2.1a ✅** / **2.1b ✅ 3/3** / **2.1c ✅ 完整交付** (Session 6 末) / **MVP 2.2 Data Lineage (U3) ✅ Sub1+Sub2 全交付** / **MVP 2.3 Sub1 5 PR ✅** (Session 7 PR #11 `a0e01db` ALTER migration + ColumnSpec + ADR-007 / PR #13 `89a925e` PlatformBacktestRunner + DBBacktestRegistry + SerialBacktestExecutor + U3 lineage 集成 + 30 tests + 9 P1 review fixes / PR #14 `f57cf2c` SDK 成熟度 1: direction_provider + ParquetBaselineLoader + BacktestCacheLoader + soft deprecation + 10 tests + 4 P1/P2 review fixes / Session 8 PR #15 `a731caa` SDK 成熟度 2 infra-only: BacktestResult.engine_artifacts + InMemoryBacktestRegistry + LSP abstract `**kwargs` 扩 + `__post_init__` 浅拷贝隔离 + max_history=0 bug 修 + 14 tests + 6 HIGH/MEDIUM reviewer fixes / **Session 8 PR #16 `dc6a13c` SDK 成熟度 3 + run_backtest.py 迁 SDK: engine_config_builder + signal_config_builder kwargs (修 PR B SN=0 silent bug) + _build_engine_config static→instance + signal_config_builder=None UserWarning 对称 direction_provider + run_backtest.py run_with_yaml 迁 Platform SDK (LIVE_PT ad-hoc 借语义 / closure data_loader cache+DB / builder 注入完整 YAML 14+8 字段) + 手动 smoke bit-identical (pre=post=Sharpe 0.38) + 14 tests + 10 HIGH/MEDIUM/LOW reviewer fixes**). **铁律 10b 全面落地** / **Session 6 governance 升级**: 铁律 42 PR 分级审查制 + LL-055/056/057/058 / **Session 7+8 workflow 成熟**: LL-059 9 步闭环实战 **5 次** (PR A/B/C1/C2/C3), AI 自 merge 0 用户接触.
- ⬜ **Wave 2 剩余** (~1-2 周): **MVP 2.3 Sub1 PR C4** (`scripts/regression_test.py` + `scripts/research/profile_backtest.py` 迁 Platform SDK, Sub1 硬门 #5 #6 最终拼图 — regression 迁需保 max_diff=0 铁律 15) + **MVP 2.3 Sub2** (批 2/3 research scripts 20+ 全量迁 SDK, ~1 周) + **MVP 2.3 Sub3** (U1 Parity 实盘对齐, SignalPipeline 统一 research & PT, ~1 周)

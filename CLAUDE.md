...
- **调度链路**: 09:00-14:55 **intraday-risk-check Celery Beat `*/5 9-14 * * 1-5`** (MVP 3.1 批 2, Session 29-30, 72 trigger/日, IntradayPortfolioDrop3/5/8% + QMTDisconnectRule, Redis 24h TTL dedup fail-open) → T+1 09:31执行 → **14:30 risk-daily-check Celery Beat** (MVP 3.1 批 1+3, Session 28+30, PMSRule L1/L2/L3 + CircuitBreakerRule Hybrid adapter 方案 C) → 15:10对账 → 16:15数据拉取 → 16:25预检 → 16:30因子+信号 → 17:30 moneyflow+factor_health → **17:35 pt_audit 主动守门** → **18:00 DailyIC (每日增量 IC 入库 CORE 4, Session 22 Part 2)** → **18:15 IcRolling (ic_ma20/60 rolling 刷新, Session 22 Part 8)** → **18:30 DataQualityCheck (Session 26 shift 17:45→18:30, 避 dense window + 脚本硬化)** → 周五 19:00 factor-lifecycle Beat. **17:05 DailyExecuteAfterData 已永久废除 (Stage 4 Session 17, ADR-008 P0-δ 污染源)**. **铁律 11 + 17 每日 IC 全链完工** (Session 23 Part 1+2): 3 脚本分工 (compute_daily_ic / compute_ic_rolling / fast_ic_recompute) + 2 schtask wire + 实战 rehearsal 验证 GO. **MVP 3.1 Risk Framework 正式完结** (Session 30 2026-04-24, 批 1+2+3 全 merged PR #55/#57/#58/#59/#60/#61 6 PR + 1 spike #54, Celery Beat 5 schedule entries 生产激活, 首次真生产触发 2026-04-27 Monday 09:00 intraday + 14:30 daily).
...
- ⭐ **Platform Blueprint v1.8** (`docs/QUANTMIND_PLATFORM_BLUEPRINT.md`, 1760+ 行): 12 Framework + 6 升维 + 17 MVP (27-36.5 周)

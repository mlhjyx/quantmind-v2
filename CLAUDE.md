...
42. **PR 分级审查制 (Auto mode 缓冲层)** — 全局原则: AI-heavy + 真金白银生产环境下 main 直 push 是危险默认, 必须有"暂停 + revisit" buffer. 分级:
    ...
    - **PR description 必须含**: (a) 改动文件分类 (代码/测试/文档/配置); (b) 验证证据 (smoke 数字 + regression max_diff + pytest fail 数不增 vs 当前 baseline, 铁律 40); (c) 关联 MVP / Sprint / 设计稿; (d) 风险 + 回滚方案
...
- ⭐ **Platform Blueprint v1.11** (`docs/QUANTMIND_PLATFORM_BLUEPRINT.md`): 12 Framework + 6 升维 + 17 MVP (27-36.5 周)
...
- 🟡 **MVP 3.3 Signal-Exec 75% 完成** (Session 37-39, 跨 4 sessions, ~10 PR 累计): **batch 1 ✅** PR #107 (PlatformSignalPipeline.compose/generate concrete + S1 wrap, regression max_diff=0) / **batch 2.1 ✅** PR #108 (PlatformOrderRouter SDK + 整手 + cash buffer + idempotent order_id sha256) / **batch 3 ✅** PR #109 (StubExecutionAuditTrail no-op + Router audit hook wire 铁律 33) / **batch 2.2 = Step 2 ✅** PR #110 (`_run_sdk_parity_dryrun` SDK vs legacy warn-only dual-run + read-only 1 周观察期) / **batch 2.2 = Step 2.5 ✅** PR #111 (STRICT mode env-gated `SignalPathDriftError(RuntimeError)` raise, `_is_sdk_parity_strict()` + `_STRICT_TRUTHY` frozenset) / **batch 2.2 verification ✅** PR #112 (`scripts/sdk_parity_scan.py` multi-date pre-flip 验证 14/14 trade_dates 4-07~4-24 bit-identical max_w_diff=0.00e+00 + STRICT flip 真切换 LIVE via `setx SDK_PARITY_STRICT true` Windows User env). LL-082/083/084 (Session 38-39 早段) + LL-085/086 (Session 39 加时) 入册. **剩 25%** = batch 2.3 = **Stage 3.0 真切换** (`signal_service.write_signals` 走 SDK Order, 删 legacy compose), 解锁 = 1 周连续 STRICT OK (Tuesday 4-28 16:30 ~ Monday 5-04). LL-059 9 步闭环跨 sessions 70+ → 77+ (今晚 +3).
- 🟡 **Session 40 生产值守 (2026-04-28 Tuesday)**: (1) 10:00 钉钉 false-positive 告警 → PR #113 撤销 `qm:qmt:status` stream check (transition-only event ≠ heartbeat, LL-087 入册, -197 行 cleanup, "Services Recovered" 钉钉送达). (2) 14:30 schtasks audit 发现 **DailySignal Mon 4-27 16:30 LastResult=0xC0000142** STATUS_DLL_INIT_FAILED → klines_daily/daily_basic/factor_values 4-27 全 0 行 (Sunday 维护 aftermath transient, 当前 import 干净). 生产值守 backfill 4-27 数据 + 因子 (run_paper_trading signal --date 2026-04-27 --dry-run) 防 Tuesday 16:30 STRICT 首跑撞 lookback 缺口. LL-059 闭环 78+ (今日 PR #113).
- ⬜ **Wave 4** (3-4 周): Observability + Performance Attribution + CI/CD + Backup & DR

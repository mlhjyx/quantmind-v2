...
2. 检查关键文档 (AGENTS.md/IRONLAWS.md/SYSTEM_STATUS.md) 是否同步
...
def main():
    ...
    if "git commit" in command:
        try:
            ...
            py_files = [
                f
                for f in (staged_py.stdout or "").strip().split("\n")
                if f.strip()
            ...
            if py_files:
                ...
                if result.returncode != 0:
                    ...
                    errors.append(
                        f"ruff check FAILED ({len(output.strip().split(chr(10)))} issues):\n"
    ...
    try:
        ...
        staged_output = staged.stdout or ""

...
def check_docs_updated(project_root: Path) -> str | None:
    ...
    try:
        ...
        all_changed = (result.stdout or "").strip() + "\n" + (unstaged.stdout or "").strip()
        code_files = [
            f
        ...
        if len(code_files) >= 3:
            docs_updated = any(
                f in all_changed for f in ["AGENTS.md", "IRONLAWS.md", "SYSTEM_STATUS.md"]
            )
            if not docs_updated:
                return f"铁律 15/重构原则: {len(code_files)} 个代码文件变更但 AGENTS.md/IRONLAWS.md/SYSTEM_STATUS.md 未更新."
    except Exception:
        pass
    return None
...
def main():
    ...
    checklist += (
        "- [ ] ruff check 通过?\n"
        "- [ ] 相关测试运行过?\n"
        "- [ ] AGENTS.md/IRONLAWS.md/SYSTEM_STATUS.md 需要更新?\n"
        "\n"

...
但 D3-B F-D3B-7 + D3-A Step 4 修订 v2 实测发现:
  - portfolio:* Redis cache = 0 keys (qmt_data_service 26 天 silent skip)
...
本 Rule 扩 LL-081 v2 cover:
  - portfolio:* Redis cache 0 keys (即使 QMT TCP 连通) → 视 fallback 触发 → P0 alert
...
关联铁律:
  ...
  - 33 (fail-loud): cache 0 keys → 不 silent skip, actual alert
  - 41 (UTC tz-aware)
...
class RedisCacheHealthReader(Protocol):
    ...
    def get_portfolio_cache_key_count(self) -> int:
        """返 portfolio:* Redis cache key 数量.
...
class QMTFallbackTriggeredRule(RiskRule):
    ...
    与 QMTDisconnectRule 区别:
      ...
      两者并行运行, 互补 cover. D3-A Step 4 实测 4-30 14:54 真账户场景:
        QMTClient.is_connected() 状态可能 True 但 portfolio:* 已 0 keys 26 天.
    ...
    def evaluate(self, context: RiskContext) -> list[RuleResult]:
        """检查 portfolio:* Redis cache, 0 keys 返 1 RuleResult, 否则 []."""
        ...
        return [
            RuleResult(
                ...
                shares=0,  # alert_only
                reason=(
                    "QMT Data Service silent drift detected: portfolio:* "

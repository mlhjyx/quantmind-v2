...
def industry_100():
    ...
    codes = [f"{i:06d}.SZ" for i in range(100)]
    industries = ["银行"] * 20 + ["医药"] * 20 + ["电子"] * 20 + ["食品"] * 20 + ["机械"] * 20
...
class TestWLSWeightCorrectness:
    ...
    def test_wls_reduces_mcap_correlation(self, mcap_correlated_factor, ln_mcap_100, industry_100):
        """WLS中性化后因子与市值的相关性 < 0.1。"""
        corr_before = abs(mcap_correlated_factor.corr(ln_mcap_100))
        result = preprocess_neutralize(mcap_correlated_factor, ln_mcap_100, industry_100)
    ...
    def test_wls_residual_mean_near_zero(self, cross_section_100, ln_mcap_100, industry_100):
        """WLS回归残差均值接近0（回归基本性质）。"""
        result = preprocess_neutralize(cross_section_100, ln_mcap_100, industry_100)
        valid = result.dropna()
    ...
    def test_wls_large_smallcap_residual_variance(
        ...
        assert ratio < 3.0, f"大/小市值残差方差比={ratio:.2f}，WLS应减少分层差异"

    def test_wls_vs_ols_both_zero_mean(self, mcap_correlated_factor, ln_mcap_100, industry_100):
        """WLS与OLS两者残差均值都应接近0，但WLS对大市值股权重更高。"""
        result_wls = preprocess_neutralize(mcap_correlated_factor, ln_mcap_100, industry_100)
        ...
        assert corr_large < 0.20, f"WLS应较好消除大市值组内市值相关性，实际={corr_large:.3f}"
...
class TestZscoreClip:
    ...
    def test_pipeline_step5_max_abs_3(self, cross_section_100, ln_mcap_100, industry_100):
        """pipeline输出 |neutral_value| <= 3.0 (Step5生效)。"""
        _, neutral = preprocess_pipeline(cross_section_100, ln_mcap_100, industry_100)
    ...
    def test_pipeline_step5_with_extreme_outlier(
        ...
        _, neutral = preprocess_pipeline(factor_with_outlier, ln_mcap_100, industry_100)
        valid = neutral.dropna()
    ...
    def test_pipeline_clip_does_not_affect_bulk(self, cross_section_100, ln_mcap_100, industry_100):
        """clip不影响主体分布: z ∈ [-3,3] 的值占95%+。"""
        _, neutral = preprocess_pipeline(cross_section_100, ln_mcap_100, industry_100)
        valid = neutral.dropna()
        # zscore 后主体应在 [-3,3] 内，clip 不修改这些值
        pct_in_range = (valid.abs() <= 3.0).mean()
        assert pct_in_range == 1.0, "clip(±3)后所有值都在[-3,3]内"
    ...
    def test_pipeline_returns_raw_unchanged(self, cross_section_100, ln_mcap_100, industry_100):
        """pipeline 返回的 raw 值是原始值（Step5不影响raw）。"""
        raw, neutral = preprocess_pipeline(cross_section_100, ln_mcap_100, industry_100)
...
class TestNeutralizerWinsorizeK:
    ...
    def test_winsorize_k_is_5(self):
        ...
        assert _WINSORIZE_K == 5.0, f"_WINSORIZE_K 应为5.0(对齐DESIGN_V5 §4.4)，实际={_WINSORIZE_K}"
    ...
    def test_extreme_outlier_clipped_at_5sigma(self):
        ...
        assert float(valid.abs().max()) < 10.0, "5σ Winsorize后极端值不应在最终zscore中出现"

...
def test_lifecycle_live_evaluate_all() -> None:
    ...
    code = (
        ...
        "from backend.qm_platform.factor.lifecycle import PlatformLifecycleMonitor\n"
        ...
        "assert len(active) >= 3, f'expected >=3 active factors, got {len(active)}'\n"
        "# CORE 4 factors 必须在 factor_registry 非 deprecated (lifecycle 可动态在\n"
        "# active↔warning 切换, 铁律 20 IC 衰减自动降级). 不要求都 active — 过度严格\n"
        "expected_core = {'turnover_mean_20', 'volatility_20', 'bp_ratio', 'dv_ttm'}\n"
        "conn_check = get_sync_conn()\n"
        "try:\n"
        ...
        "            (list(expected_core),),\n"
        "        )\n"
        "        core_alive = {r[0]: r[1] for r in cur.fetchall()}\n"
        "finally:\n"
        "    conn_check.close()\n"
        "missing_in_registry = expected_core - set(core_alive)\n"
        "assert not missing_in_registry, f'CORE deprecated/missing: {missing_in_registry}'\n"
        "non_active_core = {n: s for n, s in core_alive.items() if s != 'active'}\n"
        "\n"
        ...
        "      f'CORE non-active (allowed): {non_active_core}')"
    )

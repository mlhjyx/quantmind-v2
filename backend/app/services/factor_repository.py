...
def load_forward_returns(
    ...
    .. deprecated:: Phase D D1 (2026-04-16)
       Use :func:`engines.ic_calculator.compute_forward_excess_returns` instead.
       该函数实现统一的 IC 口径 (T+1 入场 + CSI300 超额 + Spearman, 铁律 19),
       本函数仅保留向后兼容, 不再用于生产/测试. S2b (2026-04-15) 已迁移
       factor_onboarding 全部生产调用方, Phase D D1 添加 DeprecationWarning.

       Migration::

           from engines.ic_calculator import compute_forward_excess_returns
           fwd = compute_forward_excess_returns(conn, trade_date, horizon=5)
    ...
    """
    warnings.warn(
        "factor_repository.load_forward_returns is DEPRECATED (Phase D D1, 2026-04-16). "
        "Use engines.ic_calculator.compute_forward_excess_returns instead — "
        "it implements the unified IC convention (T+1 entry, CSI300 excess, 铁律 19). "
        "S2b (2026-04-15) already migrated all production callers; this shim "

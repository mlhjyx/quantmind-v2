...
export default function TradeExecution() {
  ...
  const { data: pendingOrders, isLoading: loadingPending, isError: errorPending } = useQuery<PendingOrder[]>({
  ...
  const { data: executionLog, isLoading: loadingLog, isError: errorLog } = useQuery<ExecutionLogItem[]>({
    queryKey: ["execution-log-today"],
    queryFn: () => apiClient.get("/execution/log?date=today").then((r) => r.data),
    staleTime: 30_000,
  });

  const { data: algoConfig, isLoading: loadingAlgo, isError: errorAlgo } = useQuery<AlgoConfig>({
    queryKey: ["execution-algo-config"],
    queryFn: () => apiClient.get("/execution/algo-config").then((r) => r.data),
    staleTime: 300_000,
  });

  // Safe fallbacks after loading
  const safePending  = pendingOrders ?? [];
  const safeLog      = executionLog  ?? [];
  const safeAlgo     = algoConfig    ?? null;

  const executingCount = safePending.filter((o) => o.status === "pending").length;
  const rejectedCount = safePending.filter((o) => o.status === "rejected").length;

  // Summary metrics derived from log
  const todayTrades = safeLog.filter((e) => e.executed_at != null).length;
  const todayAmount = safeLog.reduce((sum, e) => sum + (e.fill_price ?? 0) * e.quantity, 0);
  const avgSlippage = safeLog.filter((e) => e.slippage_bps != null).length
    ? safeLog.reduce((s, e) => s + (e.slippage_bps ?? 0), 0) / safeLog.filter((e) => e.slippage_bps != null).length
    : 0;
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-3">
        ...
        <div className="grid grid-cols-5 gap-3">
          {[
            ...
            { l: "执行模式", v: loadingAlgo ? "—" : safeAlgo ? (safeAlgo.execution_mode === "paper" ? "模拟盘" : "实盘") : "—", c: C.info },
            { l: "调仓频率", v: loadingAlgo ? "—" : safeAlgo ? (safeAlgo.rebalance_freq === "monthly" ? "月度" : safeAlgo.rebalance_freq) : "—", c: C.text1 },
          ].map((m, i) => (
        ...
        {tab === "执行中" && (
          <Card>
            ...
            {loadingPending ? (
              ...
              <div className="p-6 text-center" style={{ fontSize: 12, color: "#f87171" }}>挂单数据加载失败，请确认后端服务已启动</div>
            ) : safePending.length === 0 ? (
              <div className="p-6 text-center" style={{ fontSize: 12, color: C.text4 }}>暂无挂单</div>
            ) : (
        ...
        {tab === "历史记录" && (
          <Card>
            ...
            {loadingLog ? (
              ...
              <div className="p-6 text-center" style={{ fontSize: 12, color: "#f87171" }}>成交记录加载失败，请确认后端服务已启动</div>
            ) : safeLog.length === 0 ? (
              <div className="p-6 text-center" style={{ fontSize: 12, color: C.text4 }}>暂无成交记录</div>
            ) : (
        ...
        {tab === "算法设置" && (
          loadingAlgo ? (
            ...
            <div className="p-6 text-center" style={{ fontSize: 12, color: "#f87171" }}>算法配置加载失败，请确认后端服务已启动</div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {[
                { name: "执行模式", value: safeAlgo.execution_mode === "paper" ? "模拟盘 (Paper)" : "实盘 (Live)", active: true, desc: "当前策略运行模式", extra: `v${safeAlgo.version} · ${safeAlgo.strategy_name}` },
                { name: "滑点模型", value: safeAlgo.slippage_model, active: true, desc: "订单执行滑点估算方式", extra: `${safeAlgo.slippage_bps} bps` },
                { name: "委托方式", value: safeAlgo.order_type === "market_open" ? "集合竞价" : safeAlgo.order_type, active: true, desc: "订单类型设置", extra: `Top-${safeAlgo.top_n} · 换手上限 ${(safeAlgo.turnover_cap * 100).toFixed(0)}%` },
                { name: "风险参数", value: `单股 ${(safeAlgo.max_single_weight * 100).toFixed(0)}%`, active: true, desc: "仓位权重约束", extra: `行业 ${(safeAlgo.max_industry_weight * 100).toFixed(0)}% · 现金 ${(safeAlgo.cash_buffer * 100).toFixed(0)}%` },
              ].map((algo) => (
